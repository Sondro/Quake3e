...
static void R_SetFarClip(void )
{
	...
	// if not drawing the world (icons, menus, etc)
...
void R_SetupFrustum (viewParms_t *dest, float xmin, float xmax, float ymax, float zProj, float zFar, float stereoSep)
{
	...
	{
		// In stereo drawing, due to the modification of the projection matrix, dest->or.origin is not the
		// actual origin that we're drawing so offset the tip of the view pyramid.
...
void R_SetupProjection(viewParms_t *dest, float zProj, float zFar, qboolean computeFrustum)
{
	...
	/*
	 * offset the view origin of the viewer for stereo drawing 
...
qboolean R_GetPortalOrientations(drawSurf_t *drawSurf, int entityNum, 
	...
	// if we didn't locate a portal entity, don't draw anything.
	// We don't want to just treat it as a mirror, because without a
	// portal entity the host won't have communicated a proper entity set
	// in the snapshot

	// unfortunately, with local movement prediction it is easily possible
	// to see a surface before the host has communicated the matching
...
Returns qtrue if another view has been drawed
========================
*/
...
void R_SortDrawSurfs(drawSurf_t *drawSurfs, int numDrawSurfs ) {
	...
	// skip pass through drawing if drawing a shadow map
	...
	// may cause another view to be drawed first
...
static void R_AddEntitySurface (int entityNum)
{
	...
	//
	if ((ent->e.drawfx & RF_FIRST_PERSON) && (tr.viewParms.flags & VPF_NOVIEWMODEL)) {
		return;
	}
	...
	case RT_RAIL_RINGS:
		...
		if ((ent->e.drawfx & RF_THIRD_PERSON) && !tr.viewParms.isPortal) {
			return;
		}
...
void R_DrawPshadowMaps(const refdef_t *fd)
{
	...
	for (i = 0; i < tr.refdef.num_entities; i++)
	{
		...
		if((ent->e.drawfx & (RF_FIRST_PERSON | RF_NOSHADOW)))
			continue;

		//if((ent->e.drawfx & RF_THIRD_PERSON))
			//continue;
...
void R_DrawCubemapSide(int cubemapIndex, int cubemapSide, qboolean subscene )
{
	...
	refdef.x = 0;
	refdef.y = 0;
	refdef.width = tr.drawCubeFbo->width;
	refdef.height = tr.drawCubeFbo->height;
	...
	if (!subscene)
	{
		...
		// FIXME: sun shadows aren't drawed correctly in cubemaps
		// fix involves changing r_FBufScale to fit smaller cubemap image size, or drawing cubemap to framebuffer first
		if(0) //(glRefConfig.framebufferObject && r_sunlightMode->integer && (r_forceSun->integer || tr.sunShadows))
		{
			R_DrawSunShadowMaps(&refdef, 0);
			R_DrawSunShadowMaps(&refdef, 1);
			R_DrawSunShadowMaps(&refdef, 2);
			R_DrawSunShadowMaps(&refdef, 3);
		}
	}
	...
	parms.viewportX = 0;
	parms.viewportY = 0;
	parms.viewportWidth = tr.drawCubeFbo->width;
	parms.viewportHeight = tr.drawCubeFbo->height;
	...
	parms.targetFbo = tr.drawCubeFbo;
