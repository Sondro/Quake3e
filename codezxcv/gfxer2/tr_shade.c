...
#if 0
	// if in greyscale drawing mode turn all color values into greyscale.
...
static void RB_IterateStagesGeneric(shaderCommands_t *input )
{
	...
	qboolean drawToCubemap = tr.drawCubeFbo && glState.currentFBO == tr.drawCubeFbo;
	...
	for (stage = 0; stage < MAX_SHADER_STAGES; stage++ )
	{
...

		{
			...
			if (drawToCubemap)
			{
				// force specular to nonmetal if drawing cubemaps
				if (r_pbr->integer)
		...
		if (!(tr.viewParms.flags & VPF_NOCUBEMAPS) && input->cubemapIndex && r_cubeMapping->integer)
		{
			...
			// FIXME: cubemap image could be NULL if cubemap isn't gfxer or loaded
			if (cubemap->image)
...
static void RB_DrawShadowmap(shaderCommands_t *input )
{
	int deformGen;
	vec5_t deformParams;
...
void RB_StageIteratorGeneric(void )
{
	...
	//
	// draw depth if in depthfill mode
	//
	if (backEnd.depthFill)
	{
	...
	//
	// draw shadowmap if in shadowmap mode
	//
	if (backEnd.viewParms.flags & VPF_SHADOWMAP)
	{
		if (input->shader->sort == SS_OPAQUE )
		{
			RB_DrawShadowmap(input );
		}
		//
		// reset polygon offset
		//
...
void RB_EndSurface(void ) {
	...
	// for debugging of sort order issues, stop drawing after a given sort value
