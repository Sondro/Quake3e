...
void GL_BindDrawbuffer(GLuint drawbuffer)
{
	if (drawbuffer != glDsaState.drawbuffer)
	{
		qglBindDrawbuffer(GL_RENDERBUFFER, drawbuffer);
		glDsaState.drawbuffer = drawbuffer;
	}
}

GLvoid APIENTRY GLDSA_NamedDrawbufferStorageEXT(GLuint drawbuffer,
	GLenum internalformat, GLsizei width, GLsizei height)
{
	GL_BindDrawbuffer(drawbuffer);
	qglDrawbufferStorage(GL_RENDERBUFFER, internalformat, width, height);
}

GLvoid APIENTRY GLDSA_NamedDrawbufferStorageMultisampleEXT(GLuint drawbuffer,
	GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height)
{
	GL_BindDrawbuffer(drawbuffer);
	qglDrawbufferStorageMultisample(GL_RENDERBUFFER, samples, internalformat, width, height);
}
...
GLvoid APIENTRY GLDSA_NamedFramebufferDrawbufferEXT(GLuint framebuffer,
	GLenum attachment, GLenum drawbuffertarget, GLuint drawbuffer)
{
	GL_BindFramebuffer(GL_FRAMEBUFFER, framebuffer);
	qglFramebufferDrawbuffer(GL_FRAMEBUFFER, attachment, drawbuffertarget, drawbuffer);
}
