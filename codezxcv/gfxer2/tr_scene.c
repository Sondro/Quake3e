...
void RE_BeginScene(const refdef_t *fd)
{
	...
	// a 3D game view, 3D status bar drawings, 3D menus, etc.
...
{
	// the next scene drawed in this frame will tack on after this one
...
Drawing a scene may require multiple views to be drawed
to handle mirrors,
@@@@@@@@@@@@@@@@@@@@@
*/
...
//	GLimp_LogComment("====== RE_DrawScene =====\n" );
	...
	if (!tr.world && !(fd->rdflags & RDF_NOWORLDMODEL ) ) {
		ri.Error (ERR_DROP, "R_DrawScene: NULL worldmodel");
	}
	...
	if(glRefConfig.framebufferObject && r_sunlightMode->integer && !(fd->rdflags & RDF_NOWORLDMODEL ) && (r_forceSun->integer || tr.sunShadows))
	{
		...
		{
			R_DrawSunShadowMaps(fd, 0);
			R_DrawSunShadowMaps(fd, 1);
			R_DrawSunShadowMaps(fd, 2);
		}
		else
		{
		...
		// only redraw last cascade if sun has changed position
	...
	// this is where dynamic cubemaps would be drawed
