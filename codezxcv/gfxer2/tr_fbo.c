...
FBO_t          *FBO_Create(const char *name, int width, int height)
{
	...
	if(width <= 0 || width > glRefConfig.maxDrawbufferSize)
	{
		ri.Error(ERR_DROP, "FBO_Create: bad width %i", width);
	}

	if(height <= 0 || height > glRefConfig.maxDrawbufferSize)
	{
...
void FBO_CreateBuffer(FBO_t *fbo, int format, int index, int multisample)
{
	...
	switch(format)
	{
		...
		case GL_RGBA32F_ARB:
			...
			pDrawBuffer = &fbo->colorBuffers[index];
		...
		case GL_DEPTH24_STENCIL8:
			...
			pDrawBuffer = &fbo->packedDepthStencilBuffer;
	...
	absent = *pDrawBuffer == 0;
	if (absent)
		qglGenDrawbuffers(1, pDrawBuffer);

	if (multisample && glRefConfig.framebufferMultisample)
		qglNamedDrawbufferStorageMultisampleEXT(*pDrawBuffer, multisample, format, fbo->width, fbo->height);
	else
		qglNamedDrawbufferStorageEXT(*pDrawBuffer, format, fbo->width, fbo->height);

	if(absent)
	{
		if (attachment == 0)
		{
			qglNamedFramebufferDrawbufferEXT(fbo->frameBuffer, GL_DEPTH_ATTACHMENT,   GL_RENDERBUFFER, *pDrawBuffer);
			qglNamedFramebufferDrawbufferEXT(fbo->frameBuffer, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, *pDrawBuffer);
		}
		else
		{
			qglNamedFramebufferDrawbufferEXT(fbo->frameBuffer, attachment, GL_RENDERBUFFER, *pDrawBuffer);
		}
	}
}
...
void FBO_Init(void)
{
	...
	// only create a draw FBO if we need to resolve MSAA or do HDR
	// otherwise just draw straight to the screen (tr.drawFbo = NULL)
	if (multisample && glRefConfig.framebufferMultisample)
	{
		tr.drawFbo = FBO_Create("_draw", tr.drawDepthImage->width, tr.drawDepthImage->height);
		FBO_CreateBuffer(tr.drawFbo, hdrFormat, 0, multisample);
		FBO_CreateBuffer(tr.drawFbo, GL_DEPTH_COMPONENT24, 0, multisample);
		R_CheckFBO(tr.drawFbo);

		tr.msaaResolveFbo = FBO_Create("_msaaResolve", tr.drawDepthImage->width, tr.drawDepthImage->height);
		FBO_AttachImage(tr.msaaResolveFbo, tr.drawImage, GL_COLOR_ATTACHMENT0, 0);
		FBO_AttachImage(tr.msaaResolveFbo, tr.drawDepthImage, GL_DEPTH_ATTACHMENT, 0);
		R_CheckFBO(tr.msaaResolveFbo);
	}
	else if (r_hdr->integer)
	{
		tr.drawFbo = FBO_Create("_draw", tr.drawDepthImage->width, tr.drawDepthImage->height);
		FBO_AttachImage(tr.drawFbo, tr.drawImage, GL_COLOR_ATTACHMENT0, 0);
		FBO_AttachImage(tr.drawFbo, tr.drawDepthImage, GL_DEPTH_ATTACHMENT, 0);
		R_CheckFBO(tr.drawFbo);
	}
	...
	if (tr.drawFbo)
	{
		GL_BindFramebuffer(GL_FRAMEBUFFER, tr.drawFbo->frameBuffer);
	...
	if (tr.screenScratchImage)
	{
		...
		FBO_AttachImage(tr.screenScratchFbo, tr.drawDepthImage, GL_DEPTH_ATTACHMENT, 0);
		R_CheckFBO(tr.screenScratchFbo);
	}

	if (tr.sunRaysImage)
	{
		tr.sunRaysFbo = FBO_Create("_sunRays", tr.drawDepthImage->width, tr.drawDepthImage->height);
		FBO_AttachImage(tr.sunRaysFbo, tr.sunRaysImage, GL_COLOR_ATTACHMENT0, 0);
		FBO_AttachImage(tr.sunRaysFbo, tr.drawDepthImage, GL_DEPTH_ATTACHMENT, 0);
	...
	if (tr.drawCubeImage)
	{
		tr.drawCubeFbo = FBO_Create("_drawCubeFbo", tr.drawCubeImage->width, tr.drawCubeImage->height);
		FBO_AttachImage(tr.drawCubeFbo, tr.drawCubeImage, GL_COLOR_ATTACHMENT0, 0);
		FBO_CreateBuffer(tr.drawCubeFbo, GL_DEPTH_COMPONENT24_ARB, 0, 0);
		R_CheckFBO(tr.drawCubeFbo);
	}
	...
	for(i = 0; i < tr.numFBOs; i++)
	{
		...
		for(j = 0; j < glRefConfig.maxColorAttachments; j++)
		{
			if(fbo->colorBuffers[j])
				qglDeleteDrawbuffers(1, &fbo->colorBuffers[j]);
		}

		if(fbo->depthBuffer)
			qglDeleteDrawbuffers(1, &fbo->depthBuffer);

		if(fbo->stencilBuffer)
			qglDeleteDrawbuffers(1, &fbo->stencilBuffer);
