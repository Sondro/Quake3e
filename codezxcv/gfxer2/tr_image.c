...
static void Upload32(byte *data, int x, int y, int width, int height, GLenum picFormat, int numMips, image_t *image, qboolean scaled)
{
	...
	if (rgba8 && !cubemap)
	{
		...
		if (type == IMGTYPE_COLORALPHA)
		{
			...
			// This corresponds to what the OpenGL1 gfxer does.
...
// Prototype for dds loader function which isn't common to both gfxers
...
static void R_CreateBuiltinImages(void ) {
	...
	if (glRefConfig.framebufferObject)
	{
		...
		tr.drawImage = R_CreateImage("_draw", NULL, width, height, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
		...
		tr.drawDepthImage  = R_CreateImage("*drawdepth",  NULL, width, height, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT24);
		...
		{
			tr.drawCubeImage = R_CreateImage("*drawCube", NULL, r_cubemapSize->integer, r_cubemapSize->integer, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE | IMGFLAG_MIPMAP | IMGFLAG_CUBEMAP, rgbFormat);
		}
	}
}
