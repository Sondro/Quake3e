...
static void InitOpenGL(void )
{
	...
	if (glConfig.vidWidth == 0 )
	{
		...
		strcpy(gfxer_buffer, glConfig.gfxer_string );
		Q_strlwr(gfxer_buffer );
...
void GfxMemInfo_f(void ) 
{
	switch (glRefConfig.memInfo)
	{
		...
		case MI_ATI:
		{
			...
			ri.Printf(PRINT_ALL, "DRAWBUFFER_FREE_MEMORY_ATI: %ikb total %ikb largest aux: %ikb total %ikb largest\n", value[0], value[1], value[2], value[3]);
		}
		break;
	}
}
