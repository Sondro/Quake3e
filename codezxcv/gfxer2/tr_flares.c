...
After all opaque surfaces have been drawed, the depth buffer is read back for
...
all other aspects of the gfxer, and is complicated by the fact that a single
frame may have multiple scenes.

RB_DrawFlares() will be called once per view (twice in a mirrored scene, potentially
...
static void RB_TestFlare(flare_t *f ) {
	...
	// if we're doing multisample drawing, read from the correct FBO
	...
	// if we're doing multisample drawing, switch to the old FBO
	if (tr.msaaResolveFbo)
	{
		FBO_Bind(oldFbo);
	}
...
==================
*/
static void RB_DrawFlare(flare_t *f ) {
	float			size;
	vec3_t			color;
	int				iColor[3];
 ...
 */
	...
	if(tr.world && f->fogNum > 0 && f->fogNum < tr.world->numfogs)
	{
		...
		// We don't need to draw the flare if colors are 0 anyways.
...
void RB_DrawFlares (void) {
	...
	// don't have influence on the drawing of these flares (i.e. RF_ gfxer flags).
