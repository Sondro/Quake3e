...
to actually draw the visible surfaces for this view
=================
*/
void RB_BeginDrawingView (void) {
	...
	if (glRefConfig.framebufferObject)
	{
		...
		// FIXME: HUGE HACK: draw to the screen fbo if we've already postprocessed the frame and aren't drawing more world
		// drawing more world check is in case of double draws, such as skyportals
		if (fbo == NULL && !(backEnd.framePostProcessed && (backEnd.refdef.rdflags & RDF_NOWORLDMODEL)))
			fbo = tr.drawFbo;

		if (tr.drawCubeFbo && fbo == tr.drawCubeFbo)
		{
	...
	if (tr.drawCubeFbo && backEnd.viewParms.targetFbo == tr.drawCubeFbo)
	{
	...
	// we will only draw a sun if there was sky drawed in this view
	backEnd.skyDrawedThisView = qfalse;
...
==================
*/
void RB_DrawDrawSurfList( drawSurf_t *drawSurfs, int numDrawSurfs ) {
	...
	for (i = 0, drawSurf = drawSurfs ; i < numDrawSurfs ; i++, drawSurf++) {
		...
		if ( entityNum != oldEntityNum ) {
			...
			if ( entityNum != REFENTITYNUM_WORLD ) {
				...
				if(backEnd.currentEntity->e.drawfx & RF_DEPTHHACK)
				{
					// hack the depth range to prevent view model from poking into walls
					depthRange = qtrue;
					
					if(backEnd.currentEntity->e.drawfx & RF_CROSSHAIR)
						isCrosshair = qtrue;
				}
			} else {
				backEnd.currentEntity = &tr.worldEntity;
...
=============
*/
void RE_StretchRaw( int x, int y, int w, int h, int cols, int rows, byte *data, int user, qboolean dirty ) {
	int			i, j;
	int			start, end;
	vec4_t quadVerts[4];
	vec2_t texCoords[4];
	...
	RE_UploadCinematic (w, h, cols, rows, data, user, dirty);
	GL_BindToTMU(tr.scratchImage[user], TB_COLORMAP);
	...
	if (glRefConfig.framebufferObject)
	{
		FBO_Bind(backEnd.framePostProcessed ? NULL : tr.drawFbo);
	}
...
void RE_UploadCinematic (int w, int h, int cols, int rows, byte *data, int user, qboolean dirty) {
	GLuint texture;

	if (!tr.scratchImage[user])
	{
		ri.Printf(PRINT_WARNING, "RE_UploadCinematic: scratch images not initialized\n");
		return;
	}

	texture = tr.scratchImage[user]->texnum;

	// if the scratchImage isn't in the format we want, specify it as a new texture
	if ( cols != tr.scratchImage[user]->width || rows != tr.scratchImage[user]->height ) {
		tr.scratchImage[user]->width = tr.scratchImage[user]->uploadWidth = cols;
		tr.scratchImage[user]->height = tr.scratchImage[user]->uploadHeight = rows;
...
const void	*RB_DrawSurfs( const void *data ) {
	...
	if (glRefConfig.framebufferObject && !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && (r_depthPrepass->integer || isShadowView))
	{
		...
		RB_DrawDrawSurfList( cmd->drawSurfs, cmd->numDrawSurfs );
		...
		if (!isShadowView)
		{
			if (tr.msaaResolveFbo)
			{
				...
				FBO_FastBlit(tr.drawFbo, NULL, tr.msaaResolveFbo, NULL, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
			}
			else if (tr.drawFbo == NULL && tr.drawDepthImage)
			{
				// If we're drawing directly to the screen, copy the depth to a texture
				// This is incredibly slow on Intel Graphics, so just skip it on there
				if (!glRefConfig.intelGraphics)
					qglCopyTextureSubImage2DEXT(tr.drawDepthImage->texnum, GL_TEXTURE_2D, 0, 0, 0, 0, 0, glConfig.vidWidth, glConfig.vidHeight);
			}
			...
			if (tr.hdrDepthFbo)
			{
				...
				FBO_BlitFromTexture(tr.drawDepthImage, srcTexCoords, NULL, tr.hdrDepthFbo, NULL, NULL, NULL, 0);
			}
			...
			if (r_sunlightMode->integer && backEnd.viewParms.flags & VPF_USESUNLIGHT)
			{
				...
				GL_BindToTMU(tr.drawDepthImage, TB_COLORMAP);
	...
	if (glRefConfig.framebufferObject && tr.drawCubeFbo && backEnd.viewParms.targetFbo == tr.drawCubeFbo)
	{
		cubemap_t *cubemap = &tr.cubemaps[backEnd.viewParms.targetFboCubemapIndex];
...
const void *RB_ClearDepth(const void *data)
{
	...
	if (glRefConfig.framebufferObject)
	{
		if (!tr.drawFbo || backEnd.framePostProcessed)
		{
			FBO_Bind(NULL);
		}
		else
	...
	if (glRefConfig.framebufferObject)
	{
		if (!backEnd.framePostProcessed)
		{
			if (tr.msaaResolveFbo && r_hdr->integer)
			{
				...
				FBO_FastBlit(tr.drawFbo, NULL, tr.msaaResolveFbo, NULL, GL_COLOR_BUFFER_BIT, GL_NEAREST);
				FBO_FastBlit(tr.msaaResolveFbo, NULL, NULL, NULL, GL_COLOR_BUFFER_BIT, GL_NEAREST);
			}
			else if (tr.drawFbo)
			{
				FBO_FastBlit(tr.drawFbo, NULL, NULL, NULL, GL_COLOR_BUFFER_BIT, GL_NEAREST);
			}
		}
	}
...
const void *RB_PostProcess(const void *data)
{
	...
	if (tr.msaaResolveFbo)
	{
		...
		FBO_FastBlit(tr.drawFbo, NULL, tr.msaaResolveFbo, NULL, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT, GL_NEAREST);
	...
	if (0)
	{
		...
		FBO_BlitFromTexture(tr.drawDepthImage, NULL, NULL, NULL, dstBox, NULL, NULL, 0);
	...
	if (r_cubeMapping->integer && tr.numCubemaps)
	{
		...
		if (cubemapIndex)
		{
			...
			//FBO_BlitFromTexture(tr.drawCubeImage, NULL, NULL, NULL, dstBox, &tr.testcubeShader, NULL, 0);
...
const void *RB_ExportCubemaps(const void *data)
{
	...
	if (cmd)
	{
		...
		for (i = 0; i < tr.numCubemaps; i++)
		{
			...
			for (j = 0; j < 6; j++)
			{
				FBO_AttachImage(tr.drawCubeFbo, cubemap->image, GL_COLOR_ATTACHMENT0_EXT, j);
...
/*
====================
RB_ExecuteDrawCommands
====================
*/
void RB_ExecuteDrawCommands( const void *data ) {
	int		t1, t2;
