...
static qboolean ParseShader( const char **text )
{
	...
	while( 1 )
	{
		...
		// skip stuff that only q3map or the host needs
...
but before the frame is drawed. This will, for the duration of one frame, cause drawsurfaces
to be drawed with bad shaders. To fix this, need to go through all draw commands and fix
sortedIndex.
==============
*/
static void FixDrawCommandList( int newShader ) {
	drawCommandList_t	*cmdList = &backEndData->commands;
...
static void SortNewShader( void ) {
	...
	// Arnout: fix drawcommandlist
	// https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=493
	FixDrawCommandList( i+1 );
...
If vertex lighting is enabled, only draw a single
...
for 2D drawing unless an explicit shader is found
