...
/*
** GL_SelectUserTexture
*/
#ifndef VULKAN_ON_Make
static void GL_SelectUserTexture( int unit )
{
	...
	if ( unit >= glConfig.numTextureUnits )
	{
		ri.Error( ERR_DROP, "GL_SelectUserTexture: unit = %i", unit );
	}

	qglUserActiveTextureARB( GL_TEXTURE0_ARB + unit );
...
#ifndef VULKAN_ON_Make
void GL_UserState( int unit, unsigned stateBits )
{
	unsigned diff = stateBits ^ glState.glUserStateBits[ unit ];

	if ( diff == 0 )
	{
		if ( stateBits )
		{
			GL_SelectUserTexture( unit );
		}
		return;
	}
	...
	if ( diff & CLS_COLOR_ARRAY )
	{
		if ( stateBits & CLS_COLOR_ARRAY )
			qglEnableUserState( GL_COLOR_ARRAY );
		else
	...
	if ( diff & CLS_NORMAL_ARRAY )
	{
		if ( stateBits & CLS_NORMAL_ARRAY )
			qglEnableUserState( GL_NORMAL_ARRAY );
		else
	...
	if ( diff & CLS_TEXCOORD_ARRAY )
	{
		if ( stateBits & CLS_TEXCOORD_ARRAY )
			qglEnableUserState( GL_TEXTURE_COORD_ARRAY );
		else
			qglDisableUserState( GL_TEXTURE_COORD_ARRAY );
	}

	glState.glUserStateBits[ unit ] = stateBits;
}
#endif
...
==================
*/
static void RB_DrawDrawSurfList( drawSurf_t *drawSurfs, int numDrawSurfs ) {
...
#ifdef VULKAN_ON_Make
		if ( vk.drawPassIndex == DRAW_PASS_SCREENMAP && entityNum != REFENTITYNUM_WORLD && backEnd.refdef.entities[ entityNum ].e.drawfx & RF_DEPTHHACK ) {
			continue;
		}
#endif
		//
...
==================
*/
static void RB_DrawLitSurfList( dlight_t* dl ) {
	shader_t		*shader, *oldShader;
	int				fogNum;
	int				entityNum, oldEntityNum;
#ifndef VULKAN_ON_Make
...
void RE_StretchRaw( int x, int y, int w, int h, int cols, int rows, byte *data, int user, qboolean dirty ) {
	...
	tr.cinematicShader->stages[0]->bundle[0].image[0] = tr.scratchImage[user];
...
void RE_UploadCinematic( int w, int h, int cols, int rows, byte *data, int user, qboolean dirty ) {
	...
	if ( !tr.scratchImage[ user ] ) {
		tr.scratchImage[ user ] = R_CreateImage( va( "*scratch%i", user ), NULL, data, cols, rows, IMGFLAG_CLAMPTOEDGE | IMGFLAG_RGB | IMGFLAG_NOSCALE );
	}

	image = tr.scratchImage[ user ];
