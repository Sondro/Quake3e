...
//
// Vulkan API functions used by the gfxer.
//
...
PFN_vkCmdBeginDrawPass						qvkCmdBeginDrawPass;
...
PFN_vkCmdEndDrawPass							qvkCmdEndDrawPass;
...
PFN_vkCreateDrawPass							qvkCreateDrawPass;
...
PFN_vkDestroyDrawPass							qvkDestroyDrawPass;
...
// forward declaration
VkPipeline create_pipeline( const Vk_Pipeline_Def *def, drawPass_t drawPassIndex );
...
static void vk_create_draw_passes( void )
{
	...
	else
	{
		...
		attachments[0].storeOp = VK_ATTACHMENT_STORE_OP_STORE;   // needed for next draw pass
		attachments[0].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		attachments[0].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
	...
	desc.sType = VK_STRUCTURE_TYPE_DRAW_PASS_CREATE_INFO;
	desc.pNext = NULL;
	desc.flags = 0;
	...
	if( r_fbo->integer == 0 )
	{
		...
		VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.main ) );
		SET_OBJECT_NAME( vk.draw_pass.main, "draw pass - main", VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );
	...
	if( r_bloom->integer ) {
		...
		VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.post_bloom ) );
		SET_OBJECT_NAME( vk.draw_pass.post_bloom, "draw pass - post_bloom", VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );
		...
		VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.bloom_extract ) );
		SET_OBJECT_NAME( vk.draw_pass.bloom_extract, "draw pass - bloom_extract", VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );

		for( i = 0; i < ARRAY_LEN( vk.draw_pass.blur ); i++ )
		{
			VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.blur[i] ) );
			SET_OBJECT_NAME( vk.draw_pass.blur[i], va( "draw pass - blur %i", i ), VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );
		}
	}
	...
	if( vk.capture.image )
	{
		...
		VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.capture ) );
		SET_OBJECT_NAME( vk.draw_pass.capture, "draw pass - capture", VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );
	}
	...
	VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.gamma ) );

	SET_OBJECT_NAME( vk.draw_pass.gamma, "draw pass - gamma", VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );
	...
	VK_CHECK( qvkCreateDrawPass( device, &desc, NULL, &vk.draw_pass.screenmap ) );

	SET_OBJECT_NAME( vk.draw_pass.screenmap, "draw pass - screenmap", VK_DEBUG_REPORT_OBJECT_TYPE_DRAW_PASS_EXT );
}
...
static const char *gfxer_name( const VkPhysicalDeviceProperties *props ) {
...
static void init_vulkan_library( void )
{
	...
	INIT_DEVICE_FUNCTION(vkCmdBeginDrawPass)
	...
	INIT_DEVICE_FUNCTION(vkCmdEndDrawPass)
	...
	INIT_DEVICE_FUNCTION(vkCreateDrawPass)
	...
	INIT_DEVICE_FUNCTION(vkDestroyDrawPass)
...
static void vk_create_framebuffers( void )
{
	...
	{
		desc.drawPass = vk.draw_pass.main;
		desc.attachmentCount = 2;
		if( r_fbo->integer == 0 )
		{
		...
		{
			// same framebuffer configuration for main and post-bloom draw passes
			if( n == 0 )
			{
			...
			// gamma correction
			desc.drawPass = vk.draw_pass.gamma;
			desc.attachmentCount = 1;
			desc.width = gls.windowWidth;
			desc.height = gls.windowHeight;
			attachments[0] = vk.swapchain_image_views[n];
	...
	{
		// screenmap
		desc.drawPass = vk.draw_pass.screenmap;
		desc.attachmentCount = 2;
		...
		if( vk.capture.image != VK_NULL_HANDLE )
		{
			...
			desc.drawPass = vk.draw_pass.capture;
			desc.pAttachments = attachments;
			desc.attachmentCount = 1;
		...
		if( r_bloom->integer )
		{
			...
			desc.drawPass = vk.draw_pass.bloom_extract;
			desc.width = width;
			desc.height = height;
			...
			for( n = 0; n < ARRAY_LEN( vk.framebuffers.blur ); n += 2 )
			{
				...
				desc.drawPass = vk.draw_pass.blur[n];
				desc.width = width;
				desc.height = height;
...
static void vk_create_sync_primitives( void ) {
	...
	for( i = 0; i < NUM_COMMAND_BUFFERS; i++ )
	{
		...
		VK_CHECK( qvkCreateSemaphore( vk.device, &desc, NULL, &vk.tess[i].drawing_finished ) );

		fence_desc.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
		fence_desc.pNext = NULL;
		fence_desc.flags = VK_FENCE_CREATE_SIGNALED_BIT; // so it can be used to start drawing

		VK_CHECK( qvkCreateFence( vk.device, &fence_desc, NULL, &vk.tess[i].drawing_finished_fence ) );
		...
		SET_OBJECT_NAME( vk.tess[i].drawing_finished, va( "drawing_finished semaphore %i", i ), VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT );
		SET_OBJECT_NAME( vk.tess[i].drawing_finished_fence, va( "drawing_finished fence %i", i ), VK_DEBUG_REPORT_OBJECT_TYPE_FENCE_EXT );
	}
}
...
static void vk_destroy_sync_primitives( void  ) {
	...
	for( i = 0; i < NUM_COMMAND_BUFFERS; i++ ) {
		...
		qvkDestroySemaphore( vk.device, vk.tess[i].drawing_finished, NULL );
		qvkDestroyFence( vk.device, vk.tess[i].drawing_finished_fence, NULL );
...
static void vk_destroy_draw_passes( void );
...
static void vk_set_draw_scale( void )
{
	if( gls.windowWidth != glConfig.vidWidth || gls.windowHeight != glConfig.vidHeight )
	{
		if( r_drawScale->integer > 0 )
		{
			int scaleMode = r_drawScale->integer - 1;
			if( scaleMode & 1 )
			{
				// preserve aspect ratio(black bars on sides)
				float windowAspect =(float) gls.windowWidth /(float) gls.windowHeight;
				float drawAspect =(float) glConfig.vidWidth /(float) glConfig.vidHeight;
				if( windowAspect >= drawAspect )
				{
	...
	if( r_fbo->integer && r_ext_supersample->integer && !r_drawScale->integer )
	{
		vk.blitFilter = GL_LINEAR;
	}
}
...
void vk_initialize( void )
{
	...
	if( props.vendorID == 0x1002 ) {
		...
		// we can't draw to offscreen presentation surfaces on nvidia
		vk.offscreenDraw = qfalse;
		vendor_name = "NVIDIA";
	...
	Q_strncpyz( glConfig.gfxer_string, gfxer_name( &props ), sizeof( glConfig.gfxer_string ) );

	SET_OBJECT_NAME((intptr_t)vk.device, glConfig.gfxer_string, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT );
	...
	vk.drawPassIndex = DRAW_PASS_MAIN; // default draw pass
...
static void vk_destroy_draw_passes( void )
{
	...
	if( vk.draw_pass.main != VK_NULL_HANDLE ) {
		qvkDestroyDrawPass( vk.device, vk.draw_pass.main, NULL );
		vk.draw_pass.main = VK_NULL_HANDLE;
	}

	if( vk.draw_pass.bloom_extract != VK_NULL_HANDLE ) {
		qvkDestroyDrawPass( vk.device, vk.draw_pass.bloom_extract, NULL );
		vk.draw_pass.bloom_extract = VK_NULL_HANDLE;
	}

	for( i = 0; i < ARRAY_LEN( vk.draw_pass.blur ); i++ ) {
		if( vk.draw_pass.blur[i] != VK_NULL_HANDLE ) {
			qvkDestroyDrawPass( vk.device, vk.draw_pass.blur[i], NULL );
			vk.draw_pass.blur[i] = VK_NULL_HANDLE;
		}
	}

	if( vk.draw_pass.post_bloom != VK_NULL_HANDLE ) {
		qvkDestroyDrawPass( vk.device, vk.draw_pass.post_bloom, NULL );
		vk.draw_pass.post_bloom = VK_NULL_HANDLE;
	}

	if( vk.draw_pass.screenmap != VK_NULL_HANDLE ) {
		qvkDestroyDrawPass( vk.device, vk.draw_pass.screenmap, NULL );
		vk.draw_pass.screenmap = VK_NULL_HANDLE;
	}

	if( vk.draw_pass.gamma != VK_NULL_HANDLE ) {
		qvkDestroyDrawPass( vk.device, vk.draw_pass.gamma, NULL );
		vk.draw_pass.gamma = VK_NULL_HANDLE;
	}

	if( vk.draw_pass.capture != VK_NULL_HANDLE ) {
		qvkDestroyDrawPass( vk.device, vk.draw_pass.capture, NULL );
		vk.draw_pass.capture = VK_NULL_HANDLE;
	}
}
...
void vk_create_post_process_pipeline( int program_index, uint32_t width, uint32_t height )
{
	...
	switch( program_index ) {
		case 1: // bloom extraction
			...
			drawpass = vk.draw_pass.bloom_extract;
		...
		case 2: // final bloom blend
			...
			drawpass = vk.draw_pass.post_bloom;
...
void vk_create_blur_pipeline( uint32_t index, uint32_t width, uint32_t height, qboolean horizontal_pass )
{
	...
	create_info.drawPass = vk.draw_pass.blur[ index ];
	create_info.subpass = 0;
	create_info.basePipelineHandle = VK_NULL_HANDLE;
	create_info.basePipelineIndex = -1;
...
VkPipeline create_pipeline( const Vk_Pipeline_Def *def, drawPass_t drawPassIndex ) {
	...
	multisample_state.rasterizationSamples =(vk.drawPassIndex == DRAW_PASS_SCREENMAP) ? vk.screenMapSamples : vkSamples;
	...
	if( drawPassIndex == DRAW_PASS_SCREENMAP )
		create_info.drawPass = vk.draw_pass.screenmap;
	else
		create_info.drawPass = vk.draw_pass.main;
...
VkPipeline vk_gen_pipeline( uint32_t index ) {
	if( index < vk.pipelines_count ) {
		...
		if( pipeline->handle[ vk.drawPassIndex ] == VK_NULL_HANDLE )
			pipeline->handle[ vk.drawPassIndex ] = create_pipeline( &pipeline->def, vk.drawPassIndex );
		return pipeline->handle[ vk.drawPassIndex ];
	} else {
		return VK_NULL_HANDLE;
	}
}
...
static void get_viewport_rect(VkRect2D *r)
{
	if( backEnd.projection2D )
	{
		r->offset.x = 0;
		r->offset.y = 0;
		r->extent.width = vk.drawWidth;
		r->extent.height = vk.drawHeight;
	}
	else
	{
		r->offset.x = backEnd.viewParms.viewportX * vk.drawScaleX;
		r->offset.y = vk.drawHeight -(backEnd.viewParms.viewportY + backEnd.viewParms.viewportHeight) * vk.drawScaleY;
		r->extent.width =(float)backEnd.viewParms.viewportWidth * vk.drawScaleX;
		r->extent.height =(float)backEnd.viewParms.viewportHeight * vk.drawScaleY;
	}
}
...
void vk_clear_color( const vec4_t color ) {
	...
	//		 It is recommended you use DrawPass LOAD_OP_CLEAR on Attachments prior to any Draw."
	//
	// NOTE: we don't use LOAD_OP_CLEAR for color attachment when we begin drawpass
...
static void vk_begin_draw_pass( VkDrawPass drawPass, VkFramebuffer frameBuffer, qboolean clearValues, uint32_t width, uint32_t height )
{
	VkDrawPassBeginInfo draw_pass_begin_info;
	VkClearValue clear_values[3];

	// Begin draw pass.

	draw_pass_begin_info.sType = VK_STRUCTURE_TYPE_DRAW_PASS_BEGIN_INFO;
	draw_pass_begin_info.pNext = NULL;
	draw_pass_begin_info.drawPass = drawPass;
	draw_pass_begin_info.framebuffer = frameBuffer;
	draw_pass_begin_info.drawArea.offset.x = 0;
	draw_pass_begin_info.drawArea.offset.y = 0;
	draw_pass_begin_info.drawArea.extent.width = width;
	draw_pass_begin_info.drawArea.extent.height = height;
...
#endif
		draw_pass_begin_info.clearValueCount = vk.msaaActive ? 3 : 2;
		draw_pass_begin_info.pClearValues = clear_values;

		vk_world.dirty_depth_attachment = 0;
	} else {
		draw_pass_begin_info.clearValueCount = 0;
		draw_pass_begin_info.pClearValues = NULL;
	}

	qvkCmdBeginDrawPass( vk.cmd->command_buffer, &draw_pass_begin_info, VK_SUBPASS_CONTENTS_INLINE );
}


void vk_begin_main_draw_pass( void )
{
	VkFramebuffer frameBuffer = vk.framebuffers.main[ vk.swapchain_image_index ];

	vk.drawPassIndex = DRAW_PASS_MAIN;

	vk.drawWidth = glConfig.vidWidth;
	vk.drawHeight = glConfig.vidHeight;

	//vk.drawScaleX =(float)vk.drawWidth /(float)glConfig.vidWidth;
	//vk.drawScaleY =(float)vk.drawHeight /(float)glConfig.vidHeight;
	vk.drawScaleX = vk.drawScaleY = 1.0f;

	vk_begin_draw_pass( vk.draw_pass.main, frameBuffer, qtrue, vk.drawWidth, vk.drawHeight );
}


void vk_begin_post_bloom_draw_pass( void )
{
	VkFramebuffer frameBuffer = vk.framebuffers.main[ vk.swapchain_image_index ];

	vk.drawPassIndex = DRAW_PASS_POST_BLOOM;
	...
	vk_begin_draw_pass( vk.draw_pass.post_bloom, frameBuffer, qfalse, vk.drawWidth, vk.drawHeight );
}


void vk_begin_bloom_extract_draw_pass( void )
{
	VkFramebuffer frameBuffer = vk.framebuffers.bloom_extract;

	//vk.drawPassIndex = DRAW_PASS_BLOOM_EXTRACT; // doesn't matter, we will use dedicated pipelines

	vk.drawWidth = gls.captureWidth;
	vk.drawHeight = gls.captureHeight;

	//vk.drawScaleX =(float)vk.drawWidth /(float)glConfig.vidWidth;
	//vk.drawScaleY =(float)vk.drawHeight /(float)glConfig.vidHeight;
	vk.drawScaleX = vk.drawScaleY = 1.0f;

	vk_begin_draw_pass( vk.draw_pass.bloom_extract, frameBuffer, qfalse, vk.drawWidth, vk.drawHeight );
}


void vk_begin_blur_draw_pass( uint32_t index )
{
	VkFramebuffer frameBuffer = vk.framebuffers.blur[ index ];

	//vk.drawPassIndex = DRAW_PASS_BLOOM_EXTRACT; // doesn't matter, we will use dedicated pipelines

	vk.drawWidth = gls.captureWidth /( 2 <<( index / 2 ) );
	vk.drawHeight = gls.captureHeight /( 2 <<( index / 2 ) );

	//vk.drawScaleX =(float)vk.drawWidth /(float)glConfig.vidWidth;
	//vk.drawScaleY =(float)vk.drawHeight /(float)glConfig.vidHeight;
	vk.drawScaleX = vk.drawScaleY = 1.0f;

	vk_begin_draw_pass( vk.draw_pass.blur[ index ], frameBuffer, qfalse, vk.drawWidth, vk.drawHeight );
}


void vk_begin_screenmap_draw_pass( void )
{
	...
	vk.drawPassIndex = DRAW_PASS_SCREENMAP;

	vk.drawWidth = vk.screenMapWidth;
	vk.drawHeight = vk.screenMapHeight;

	vk.drawScaleX =(float)vk.drawWidth /(float)glConfig.vidWidth;
	vk.drawScaleY =(float)vk.drawHeight /(float)glConfig.vidHeight;

	vk_begin_draw_pass( vk.draw_pass.screenmap, frameBuffer, qtrue, vk.drawWidth, vk.drawHeight );
}


void vk_end_draw_pass( void )
{
	qvkCmdEndDrawPass( vk.cmd->command_buffer );
...
void vk_begin_frame( void )
{
	...
	if( vk.frame_count++ ) // might happen during stereo drawing
		return;
	...
	if( vk.cmd->waitForFence ) {
		...
		VK_CHECK( qvkWaitForFences( vk.device, 1, &vk.cmd->drawing_finished_fence, VK_FALSE, 1e10 ) );
	} else {
	...
	VK_CHECK( qvkResetFences( vk.device, 1, &vk.cmd->drawing_finished_fence ) );
	...
	if( vk_find_screenmap_drawsurfs() ) {
		vk_begin_screenmap_draw_pass();
	} else {
		vk_begin_main_draw_pass();
	}
...
void vk_end_frame( void )
{
	...
	if( vk.fboActive )
	{
		...
		if( backEnd.screenshotMask && vk.capture.image )
		{
			...
			// draw to capture FBO
			vk_begin_draw_pass( vk.draw_pass.capture, vk.framebuffers.capture, qfalse, gls.captureWidth, gls.captureHeight );
		...
		if( !ri.CL_IsMinimized() )
		{
			...
			vk.drawWidth = gls.windowWidth;
			vk.drawHeight = gls.windowHeight;

			vk.drawScaleX = 1.0;
			vk.drawScaleY = 1.0;

			vk_begin_draw_pass( vk.draw_pass.gamma, vk.framebuffers.gamma[ vk.swapchain_image_index ], qfalse, vk.drawWidth, vk.drawHeight );
	...
	if( !ri.CL_IsMinimized() ) {
		...
		submit_info.pSignalSemaphores = &vk.cmd->drawing_finished;
	} else {
		submit_info.waitSemaphoreCount = 0;
		submit_info.pWaitSemaphores = NULL;
		submit_info.pWaitDstStageMask = NULL;
	...
	VK_CHECK( qvkQueueSubmit( vk.queue, 1, &submit_info, vk.cmd->drawing_finished_fence ) );
	...
	present_info.pWaitSemaphores = &vk.cmd->drawing_finished;
...
void vk_read_pixels( byte *buffer, uint32_t width, uint32_t height )
{
	...
	VK_CHECK( qvkWaitForFences( vk.device, 1, &vk.cmd->drawing_finished_fence, VK_FALSE, 1e12 ) );
...
qboolean vk_bloom( void )
{
	...
	if( vk.drawPassIndex == DRAW_PASS_SCREENMAP )
	{
		return qfalse;
	}
	...
	vk_end_draw_pass(); // end main

	// bloom extraction
	vk_begin_bloom_extract_draw_pass();
	...
	vk_begin_post_bloom_draw_pass(); // begin post-bloom
	{
