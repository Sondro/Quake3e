...
static void R_PerformanceCounters( void ) {
	...
	{
		ri.Printf( PRINT_ALL, "flare adds:%i tests:%i draws:%i\n", 
			backEnd.pc.c_flareAdds, backEnd.pc.c_flareTests, backEnd.pc.c_flareDraws );
	}
...
====================
*/
static void R_IssueDrawCommands( void ) {
	drawCommandList_t	*cmdList;
	...
	*(int *)(cmdList->cmds + cmdList->used) = RC_END_OF_LIST;
	...
	if( !r_skipBackEnd->integer ) {
		...
		RB_ExecuteDrawCommands( cmdList->cmds );
	}
}
...
static void *R_GetCommandBufferReserved( int bytes, int reservedBytes ) {
	...
	if( cmdList->used + bytes + sizeof( int ) + reservedBytes > MAX_DRAW_COMMANDS ) {
		if( bytes > MAX_DRAW_COMMANDS - sizeof( int ) ) {
...
#ifdef VULKAN_ON_Make
	tr.lastDrawCommand = RC_END_OF_LIST;
#endif
...
#ifdef VULKAN_ON_Make
	tr.lastDrawCommand = RC_DRAW_BUFFER;
#endif
...
{
#ifdef VULKAN_ON_Make
	if( vk.fboActive || vk.offscreenDraw )
		return qtrue;
#endif
	return qfalse;
}
