...
#include "../gfxercommon/vulkan/vulkan.h"
#include "tr_common.h"
...
#define NUM_COMMAND_BUFFERS 2	// number of command buffers / draw semaphores / framebuffer sets
...
typedef enum {
	DRAW_PASS_SCREENMAP = 0,
	DRAW_PASS_MAIN,
	DRAW_PASS_POST_BLOOM,
	DRAW_PASS_COUNT
} drawPass_t;
...
typedef struct VK_Pipeline {
	Vk_Pipeline_Def def;
	VkPipeline handle[ DRAW_PASS_COUNT ];
} VK_Pipeline_t;
...
// Called after initialization or gfxer restart
...
// Vk_Instance contains engine-specific vulkan resources that persist entire gfxer lifetime.
...
typedef struct {
	...
	struct {
		VkDrawPass main;
		VkDrawPass screenmap;
		VkDrawPass gamma;
		VkDrawPass capture;
		VkDrawPass bloom_extract;
		VkDrawPass blur[VK_NUM_BLOOM_PASSES*2]; // horizontal-vertical pairs
		VkDrawPass post_bloom;
	} draw_pass;
...
typedef struct {
	...
	// cleared by draw pass instance clear op (dirty_depth_attachment == 0).
...
// Most of the gfxer's code uses Vulkan API via function provides in this file but
