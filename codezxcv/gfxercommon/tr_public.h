...
typedef struct {
	...
	// registered before drawing any frames to prevent disk hits,
	// but they can still be registered at a later time
	// if necessary.
	//
	// BeginRegistration makes any existing media pointers invalid
	// and returns the current gl configuration, including screen width
	// and height, which can be used by the user to intelligently
	...
	// Nothing is drawn until R_DrawScene is called.
	...
	void	(*DrawScene)( const refdef_t *fd );

	void	(*SetColor)( const float *rgba );	// NULL = 1,1,1,1
	void	(*DrawStretchPic) ( float x, float y, float w, float h,
		float s1, float t1, float s2, float t2, qhandle_t hShader );	// 0 = white

	// Draw images for cinematic drawing, pass as 32 bit rgba
	void	(*DrawStretchRaw)( int x, int y, int w, int h, int cols, int rows, byte *data, int user, qboolean dirty );
	void	(*UploadCinematic)( int w, int h, int cols, int rows, byte *data, int user, qboolean dirty );
...
#ifdef __USEA3D
	void    (*A3D_DrawGeometry) (void *pVoidA3D, void *pVoidGeom, void *pVoidMat, void *pVoidGeomStatus);
#endif
...
// If the module can't init to a valid drawing state, NULL will be
// returned.
