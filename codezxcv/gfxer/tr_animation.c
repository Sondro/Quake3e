...
void R_MDRAddAnimSurfaces(trRefEntity_t *ent ) {
	...
	personalModel = (ent->e.drawfx & RF_THIRD_PERSON) && (tr.viewParms.portalView == PV_NONE);

	if (ent->e.drawfx & RF_WRAP_FRAMES )
	{
	...
	// when the surfaces are drawed, they don't need to be
	// range checked again.
	//
	...
	// figure out the current LOD of the model we're drawing, and set the lod pointer respectively.
	...
	for (i = 0 ; i < lod->numSurfaces ; i++ )
	{
		...
		if (!personalModel
		        && r_shadows->integer == 2
			&& fogNum == 0
			&& !(ent->e.drawfx & (RF_NOSHADOW | RF_DEPTHHACK ) )
