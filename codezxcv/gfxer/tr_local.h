...
#define REFENTITYNUM_BITS	12	// as we actually using only 1 bit for dlight mask in opengl1 gfxer
...
// the user game, as well as some locally derived info
typedef struct {
	refEntity_t	e;
...
typedef struct {
	...
	int			time;				// time in milliseconds for shader effects and other time dependent drawing issues
	int			rdflags;			// RDF_NOWORLDMODEL, etc

	// 1 bits will prevent the associated area from drawing at all
	...
	char		text[MAX_DRAW_STRINGS][MAX_DRAW_STRING_LENGTH];
...
// when app directly specifies a polygon, it becomes a srfPoly_t
...
*/
#define DLIGHT_BITS 1 // qboolean in opengl1 gfxer
...
// the gfxer front end should never modify glstate_t
typedef struct {
	...
	unsigned	glUserStateBits[ MAX_TEXTURE_UNITS ];
	int			currentArray;
} glstate_t;
...
typedef struct {
	...
	qboolean	skyDrawedThisView;	// flag for drawing sun

	qboolean	projection2D;	// if qtrue, drawstretchpic doesn't need to change modes
	color4ub_t	color2D;
	qboolean	doneBloom;		// done bloom this frame
	qboolean	doneSurfaces;   // done any 3d surfaces already
	trRefEntity_t	entity2D;	// currentEntity will point at this when doing 2D drawing
...
** trGlobals_t 
**
** Most gfxer globals are defined here.
...
typedef struct {
	...
	trRefEntity_t			worldEntity;		// point currentEntity at this when drawing world
	...
	// lower indexed sortedShaders must be drawed first (opaque surfaces before translucent)
	int						numShaders;
...
extern cvar_t	*r_stereoSeparation;			// separation of cameras for stereo drawing
...
extern cvar_t	*r_drawWidth;
extern cvar_t	*r_drawHeight;
extern cvar_t	*r_drawScale;

extern cvar_t	*r_dlightBacks;			// dlight non-facing surfaces for continuity

extern	cvar_t	*r_norefresh;			// bypasses the ref drawing
extern	cvar_t	*r_drawentities;		// disable/enable entity drawing
extern	cvar_t	*r_drawworld;			// disable/enable world drawing
...
extern	cvar_t	*r_lightmap;					// draw lightmaps only
extern	cvar_t	*r_vertexLight;					// vertex lighting mode for better performance

extern	cvar_t	*r_showtris;					// enables wireframe drawing of the world
...
void R_DrawView(const viewParms_t *parms );
...
void	GL_UserState(int unit, unsigned stateVector );
...
void RE_DrawScene(const refdef_t *fd );
...
GFXER BACK END COMMAND QUEUE

=============================================================
*/

#define MAX_DRAW_COMMANDS	0x80000

typedef struct {
	byte	cmds[MAX_DRAW_COMMANDS];
	int		used;
} drawCommandList_t;
...
void R_IssuePendingDrawCommands(void );
