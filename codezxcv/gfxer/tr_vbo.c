...
When it is time to draw - we sort queued items to get longest possible 
index sequence run to check if its long enough i.e. worth switching to GPU-side IBO.
So long index runs are drawed via multiple glDrawElements() calls, 
all remaining short index sequences are grouped together into single software index
which is finally drawed via single legacy index array transfer.
...
static void VBO_DrawIBOItems(void)
{
...
static void VBO_DrawSoftItems(void)
{
	const vbo_t *vbo = &world_vbo;
...
static void VBO_DrawIndexes(void)
{
	if (curr_index_bind)
	{
		VBO_DrawIBOItems();
...
static void RB_IterateStagesVBO(const shaderCommands_t *input)
{
	...
	for (i = 0; i < MAX_VBO_STAGES; i++) {
		...
		if (pStage->tessFlags & TESS_ENV0) {
			GL_UserState(0, normalMask | CLS_COLOR_ARRAY);
		} else {
			GL_UserState(0, normalMask | CLS_TEXCOORD_ARRAY | CLS_COLOR_ARRAY);
		...
		if (pStage->mtEnv) { // multitexture
			if (fp == 0) {
				...
				GL_UserState(1, CLS_TEXCOORD_ARRAY);
