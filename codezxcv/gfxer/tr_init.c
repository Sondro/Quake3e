...
static void R_InitExtensions(void)
{
	...
	Q_strncpyz(glConfig.gfxer_string,(char *)qglGetString(GL_RENDERER), sizeof(glConfig.gfxer_string));
	len = strlen(glConfig.gfxer_string);
	if(len && glConfig.gfxer_string[ len - 1 ] == '\n')
		glConfig.gfxer_string[ len - 1 ] = '\0';
	...
	if(R_HaveExtension("GL_ARB_multitexture"))
	{
		if(r_ext_multitexture->integer)
		{
			...
			qglUserActiveTextureARB = ri.GL_GetProcAddress("glUserActiveTextureARB");

			if(qglActiveTextureARB && qglUserActiveTextureARB)
			{
...
{
	//
	// initialize OS specific portions of the gfxer
	//
	...
	if(glConfig.vidWidth == 0)
	{
		...
		{
			if(r_drawScale->integer)
			{
				glConfig.vidWidth = r_drawWidth->integer;
				glConfig.vidHeight = r_drawHeight->integer;
			}
...
static void GL_SetDefaultState(void)
{
	...
	if(qglActiveTextureARB)
	{
		...
		qglDisableUserState(GL_TEXTURE_COORD_ARRAY);
	...
	qglEnableUserState(GL_VERTEX_ARRAY);

	qglDisableUserState(GL_TEXTURE_COORD_ARRAY);
	qglDisableUserState(GL_COLOR_ARRAY);
	qglDisableUserState(GL_NORMAL_ARRAY);
...
Prints persistent drawing configuration
================
*/
static void GfxInfo(void)
{
	...
	{
		ri.Printf(PRINT_ALL, "DRAW: %d x %d, MODE: %d, %d x %d %s hz:", glConfig.vidWidth, glConfig.vidHeight, mode, gls.windowWidth, gls.windowHeight, fs);
	}
	else
	{
...
static void R_Register(void)
{
	...
	ri.Cvar_SetGroup(r_bloom_blend_base, CVG_GFXER);
	...
	ri.Cvar_SetGroup(r_bloom_modulate, CVG_GFXER);
	...
	ri.Cvar_SetGroup(r_bloom_filter_size, CVG_GFXER);
	...
	ri.Cvar_SetGroup(r_greyscale, CVG_GFXER);
	...
	r_drawWidth = ri.Cvar_Get("r_drawWidth", "800", CVAR_ARCHIVE_ND | CVAR_LATCH);
	r_drawHeight = ri.Cvar_Get("r_drawHeight", "600", CVAR_ARCHIVE_ND | CVAR_LATCH);
	ri.Cvar_CheckRange(r_drawWidth, "96", NULL, CV_INTEGER);
	ri.Cvar_CheckRange(r_drawHeight, "72", NULL, CV_INTEGER);

	r_drawScale = ri.Cvar_Get("r_drawScale", "0", CVAR_ARCHIVE_ND | CVAR_LATCH);
	ri.Cvar_CheckRange(r_drawScale, "0", "4", CV_INTEGER);
	ri.Cvar_SetDescription(r_drawScale, "Scaling mode to be used with custom draw resolution:\n"
		" 0 - disabled\n"
...
refexport_t *GetRefAPI(int apiVersion, refimport_t *rimp) {
	...
	// the RE_ functions are Draw Entry points
