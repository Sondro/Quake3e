...
typedef struct frameBuffer_s {
	GLuint fbo;
	GLuint color;			// drawbuffer if multisampled
	GLuint depthStencil;	// drawbuffer if multisampled
	GLint  width;
	GLint  height;
...
void ARB_LightingPass(void )
{
	...
	GL_UserState(1, CLS_NONE );
	GL_UserState(0, CLS_TEXCOORD_ARRAY | CLS_NORMAL_ARRAY );
...
static void DrawQuad(int w, int h )
{
	...
	GL_UserState(0, CLS_TEXCOORD_ARRAY );
...
void FBO_Clean(frameBuffer_t *fb )
{
	if (fb->fbo )
	{
		...
		if (fb->multiSampled )
		{
			qglBindDrawbuffer(GL_RENDERBUFFER, 0 );
			if (fb->color )
			{
				qglDeleteDrawbuffers(1, &fb->color );
				fb->color = 0;
			}
			if (fb->depthStencil )
			{
				qglDeleteDrawbuffers(1, &fb->depthStencil );
				fb->depthStencil = 0;
			}
		}
		else
...
static void FBO_CleanDepth(void )
{
	...
	{
#ifdef DEPTH_DRAW_BUFFER
		qglDeleteDrawbuffers(1, &commonDepthStencil );
#else
		GL_BindTexture(0, 0 );
...
#ifdef DEPTH_DRAW_BUFFER
	GLuint buffer;
	qglGenDrawbuffers(1, &buffer );
	qglBindDrawbuffer(GL_RENDERBUFFER, buffer );
	if (r_stencilbits->integer == 0 )
		qglDrawbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32, width, height );
	else
		qglDrawbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width, height );
	return buffer;
#else
	GLuint tex;
...
#ifdef DEPTH_DRAW_BUFFER
		if (r_stencilbits->integer == 0 )
			qglFramebufferDrawbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, fb->depthStencil );
		else
			qglFramebufferDrawbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, fb->depthStencil );
#else
		if (r_stencilbits->integer == 0 )
...
static qboolean FBO_CreateMS(frameBuffer_t *fb, int width, int height )
{
	...
	if (nSamples <= 0 || !qglDrawbufferStorageMultisample )
	{
		return qfalse;
	}
	...
	qglGenDrawbuffers(1, &fb->color );
	qglBindDrawbuffer(GL_RENDERBUFFER, fb->color );
	while (nSamples > 0 ) {
		qglDrawbufferStorageMultisample(GL_RENDERBUFFER, nSamples, fboInternalFormat, width, height );
	...
	qglFramebufferDrawbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, fb->color );

	qglGenDrawbuffers(1, &fb->depthStencil );
	qglBindDrawbuffer(GL_RENDERBUFFER, fb->depthStencil );
	if (r_stencilbits->integer == 0 )
		qglDrawbufferStorageMultisample(GL_RENDERBUFFER, nSamples, GL_DEPTH_COMPONENT32, width, height );
	else
		qglDrawbufferStorageMultisample(GL_RENDERBUFFER, nSamples, GL_DEPTH24_STENCIL8, width, height );
...
static void FBO_Blur(const frameBuffer_t *fb1, const frameBuffer_t *fb2,  const frameBuffer_t *fb3 )
{
	...
	// apply horizontal blur - draw from FBO1 to FBO2
	...
	// apply vectical blur - draw from FBO2 to FBO3
...
static void R_Bloom_LensEffect( float alpha )
{
	...
	GL_UserState( 0, CLS_TEXCOORD_ARRAY | CLS_COLOR_ARRAY );
...
qboolean FBO_Bloom( const float gamma, const float obScale, qboolean finalStage )
{
	...
	if ( r_bloom_reflection->value )
	{
		...
		// set draw target to paired destination buffer and draw reflections
...
void QGL_SetDrawScale( qboolean verbose )
{
	...
	if ( !r_fbo->integer )
	{
		if ( verbose && r_drawScale->integer )
		{
			ri.Printf( PRINT_ALL, "...ignoring \r_drawScale due to disabled FBO\n" );
		}
		return;
	}
	...
	{
		superSampled = qtrue;
		blitFilter = GL_LINEAR; // default value for (r_drawScale==0) case
	}
...
{
	ARB_UpdatePrograms();
	QGL_SetDrawScale( qtrue );
	QGL_InitFBO();
	ri.Cvar_ResetGroup( CVG_GFXER, qtrue );
}
