...
void SCR_DrawVoipMeter(void) {
	...
	if(!cl_voipShowMeter->integer)
		...
		return;  // host doesn't support VoIP.
	else if(clc.demoplaying)
		return;  // playing back a demo.
	else if(!cl_voip->integer)
		return;  // user has VoIP support disabled.
...
This will be called twice if drawing in stereo mode
==================
*/
void SCR_DrawScreenField(stereoFrame_t stereoFrame) {
	...
	// unless they are displaying game drawings
	...
	// don't need to draw anything under it
	if(uivm && !uiFullscreen) {
		...
		case CA_CHALLENGING:
		case CA_CONNECTED:
			// connecting users will only show the connection dialog
...
void SCR_UpdateScreen(void) {
	...
	// If there is no VM, there are also no drawing commands issued. Stop the gfxer in
	// that case.
	if(uivm)
	{
		// XXX
