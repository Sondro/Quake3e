...
===========================================================================
*/
// cl.input.c  -- builds an intended movement command to send to the host
...
static void CL_MouseMove(usercmd_t *cmd)
{
	...
	// ingame FOV
	mx *= cl.appSensitivity;
	my *= cl.appSensitivity;
...
static void CL_FinishMove(usercmd_t *cmd) {
	...
	// copy the state that the app is currently sending
	cmd->weapon = cl.appUserCmdValue;

	// send the current host time so the amount of movement
	// can be determined without allowing cheating
	cmd->hostTime = cl.hostTime;
...
Create and send the command packet to the host
Including both the reliable commands and the usercmds

During normal gameplay, a user packet will contain something like:

4	sequence number
2	qport
4	hostid
4	acknowledged sequence number
4	clc.hostCommandSequence
...
void CL_WritePacket(void) {
	...
	MSG_Bitstream(&buf);
	// write the current hostId so the host
	// can tell if this is from the current gameState
	MSG_WriteLong(&buf, cl.hostId);

	// write the last message we received, which can
	// be used for delta compression, and is also used
	// to tell if we dropped a gamestate
	MSG_WriteLong(&buf, clc.hostMessageSequence);

	// write the last reliable message we received
	MSG_WriteLong(&buf, clc.hostCommandSequence);

	// write any unacknowledged userCommands
	n = clc.reliableSequence - clc.reliableAcknowledge;
	for(i = 0; i < n; i++) {
		const int index = clc.reliableAcknowledge + 1 + i;
		MSG_WriteByte(&buf, clc_userCommand);
	...
	// all the cmds will make it to the host
	...
	if(count >= 1) {
		...
		// begin a user move command
		if(cl_nodelta->integer || !cl.snap.valid || clc.demowaiting
			|| clc.hostMessageSequence != cl.snap.messageNum) {
		...
		key ^= clc.hostMessageSequence;
		// also use the last acknowledged host command in the key
		key ^= MSG_HashKey(clc.hostCommands[ clc.hostCommandSequence &(MAX_RELIABLE_COMMANDS-1) ], 32);
	...
	cl.outPackets[ packetNum ].p_hostTime = oldcmd->hostTime;
...
Called every frame to builds and sends a command packet to the host.
=================
*/
