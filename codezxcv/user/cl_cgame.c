...
===========================================================================
*/
// cl_app.c  -- user system interaction with user game
...
====================
*/
static void CL_GetCurrentSnapshotNumber(int *snapshotNumber, int *hostTime) {
	*snapshotNumber = cl.snap.messageNum;
	*hostTime = cl.snap.hostTime;
}
...
static qboolean CL_GetSnapshot(int snapshotNumber, snapshot_t *snapshot) {
	...
	snapshot->hostCommandSequence = clSnap->hostCommandNum;
	snapshot->ping = clSnap->ping;
	snapshot->hostTime = clSnap->hostTime;
	...
	// FIXME: configstring changes and host commands!!!
...
static void CL_SetUserCmdValue(int userCmdValue, float sensitivityScale) {
	cl.appUserCmdValue = userCmdValue;
	cl.appSensitivity = sensitivityScale;
}
...
===================
*/
static qboolean CL_GetHostCommand(int hostCommandNumber) {
	const char *s;
	const char *cmd;
	...
	if(clc.hostCommandSequence - hostCommandNumber >= MAX_RELIABLE_COMMANDS) {
		// when a demo record was started after the user got a whole bunch of
		// reliable commands then the user never got those first reliable commands
		if(clc.demoplaying) {
			Cmd_Clear();
			return qfalse;
		}
		Com_Error(ERR_DROP, "CL_GetHostCommand: a reliable command was cycled out");
		return qfalse;
	}

	if(clc.hostCommandSequence - hostCommandNumber < 0) {
		Com_Error(ERR_DROP, "CL_GetHostCommand: requested a command not received");
		return qfalse;
	}

	index = hostCommandNumber &(MAX_RELIABLE_COMMANDS - 1);
	s = clc.hostCommands[ index ];
	clc.lastExecutedHostCommand = hostCommandNumber;

	Com_DPrintf("hostCommand: %i : %s\n", hostCommandNumber, s);

	if(clc.hostCommandsIgnore[ index ]) {
		Cmd_Clear();
		return qfalse;
	}
...
rescan:
	...
	if(!strcmp(cmd, "disconnect")) {
		...
		// allow host to indicate why they were disconnected
		if(argc >= 2)
			Com_Error(ERR_HOSTDISCONNECT, "Host disconnected - %s", Cmd_Argv(1));
		else
			Com_Error(ERR_HOSTDISCONNECT, "Host disconnected");
	}
	...
	// the userLevelShot command is used during development
	// to generate 128*128 screenshots from the intermission
	// point of levels for the menu system to use
	// we pass it along to the app to make appropriate adjustments,
	// but we also clear the console and notify lines here
	if(!strcmp(cmd, "userLevelShot")) {
		// don't do it if we aren't running the host locally,
		// otherwise malicious remote hosts could overwrite
	...
	// we may want to put a "connect to other host" command here

	// app can now act on the command
	return qtrue;
}
...
Just adds default parameters that app doesn't need to know about
====================
*/
...
The app module is making a system call
====================
*/
static intptr_t CL_CgameSystemCalls(intptr_t *args) {
	...
	case CG_REMOVECOMMAND:
		...
		return 0;
	case CG_SENDUSERCOMMAND:
		CL_AddReliableCommand(VMA(1), qfalse);
		return 0;
	case CG_UPDATESCREEN:
		// this is used during lengthy level loading, so pump message loop
		// Com_EventLoop();	// FIXME: if a host restarts here, BAD THINGS HAPPEN!
	...
	case CG_GETHOSTCOMMAND:
		return CL_GetHostCommand(args[1]);
	...
	case CG_KEY_SETCATCHER:
		// Don't allow the app module to close the console
...
*/
	...
	default:
		Com_Error(ERR_DROP, "Bad app system trap: %ld",(long int) args[0]);
	}
	return 0;
}
...
static intptr_t QDECL CL_DllSyscall(intptr_t arg, ...) {
	...
	intptr_t	args[10]; // max.count for app
	va_list	ap;
	int i;
...
void CL_InitCGame(void) {
	...
	interpret = Cvar_VariableIntegerValue("vm_app");
	...
	if(!cgvm) {
		Com_Error(ERR_DROP, "VM_Create on app failed");
	}
	cls.state = CA_LOADING;

	// init for this gamestate
	// use the lastExecutedHostCommand instead of the hostCommandSequence
	// otherwise host commands sent just before a gamestate are dropped
	VM_Call(cgvm, 3, CG_INIT, clc.hostMessageSequence, clc.lastExecutedHostCommand, clc.userNum);

	// reset any CVAR_CHEAT cvars registered by app
	if(!clc.demoplaying && !cl_connectedToCheatHost)
		Cvar_SetCheatState();

	// we will send a usercmd this frame, which
	// will cause the host to send us the first snapshot
	cls.state = CA_PRIMED;
	...
	// have the gfxer touch all its images, so they are present
...
See if the current console command is claimed by the app
====================
*/
...
void CL_CGameGfxing(stereoFrame_t stereo) {
	VM_Call(cgvm, 3, CG_DRAW_ACTIVE_FRAME, cl.hostTime, stereo, clc.demoplaying);
#ifdef DEBUG
	VM_Debug(0);
#endif
}
...
Adjust the users view of host time.

We attempt to have cl.hostTime exactly equal the host's view
...
static void CL_AdjustTimeDelta(void) {
	...
	newDelta = cl.snap.hostTime - cls.realtime;
	deltaDelta = abs(newDelta - cl.hostTimeDelta);

	if(deltaDelta > RESET_TIME) {
		cl.hostTimeDelta = newDelta;
		cl.oldHostTime = cl.snap.hostTime;	// FIXME: is this a problem for app?
		cl.hostTime = cl.snap.hostTime;
		...
		cl.hostTimeDelta =(cl.hostTimeDelta + newDelta) >> 1;
	} else {
...
static void CL_FirstSnapshot(void) {
	...
	cl.hostTimeDelta = cl.snap.hostTime - cls.realtime;
	cl.oldHostTime = cl.snap.hostTime;

	clc.timeDemoBaseTime = cl.snap.hostTime;
...
void CL_SetCGameTime(void) {
	...
	if(cl.snap.hostTime - cl.oldFrameHostTime < 0) {
		Com_Error(ERR_DROP, "cl.snap.hostTime < cl.oldFrameHostTime");
	}
	cl.oldFrameHostTime = cl.snap.hostTime;
	...
	if(demoFreezed) {
		...
		cl.hostTimeDelta -= cls.frametime;
	} else {
		// cl_timeNudge is a user adjustable cvar that allows more
		// or less latency to be added in the interest of better
		// smoothness or better responsiveness.
		cl.hostTime = cls.realtime + cl.hostTimeDelta - CL_TimeNudge();

		// guarantee that time will never flow backwards, even if
		// hostTimeDelta made an adjustment or cl_timeNudge was changed
		if(cl.hostTime - cl.oldHostTime < 0) {
			cl.hostTime = cl.oldHostTime;
		}
		cl.oldHostTime = cl.hostTime;

		// note if we are almost past the latest frame(without timeNudge),
		// so we will try and adjust back a bit when the next snapshot arrives
		//if(cls.realtime + cl.hostTimeDelta >= cl.snap.hostTime - 5) {
		if(cls.realtime + cl.hostTimeDelta - cl.snap.hostTime >= -5) {
			cl.extrapolatedSnapshot = qtrue;
		}
	}

	// if we have gotten new snapshots, drift hostTimeDelta
	...
	// messages from the demo file until the app definitely
	...
	if(com_timedemo->integer) {
		...
		clc.timeDemoFrames++;
		cl.hostTime = clc.timeDemoBaseTime + clc.timeDemoFrames * 50;
	}

	//while(cl.hostTime >= cl.snap.hostTime) {
	while(cl.hostTime - cl.snap.hostTime >= 0) {
