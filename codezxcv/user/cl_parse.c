...
===========================================================================
*/
// cl_parse.c  -- parse a message received from the host
...
static void CL_ParseSnapshot(msg_t *msg) {
	...
	// NOTE: now sent with all host to user messages
	...
	// we will have read any new host commands in this
	// message before we got to svc_snapshot
	newSnap.hostCommandNum = clc.hostCommandSequence;

	newSnap.hostTime = MSG_ReadLong(msg);

	// if we were just unpaused, we can only *now* really let the
	// change come into effect or the user hangs.
	cl_paused->modified = qfalse;

	newSnap.messageNum = clc.hostMessageSequence;
	...
	if(newSnap.deltaNum <= 0) {
		...
		if(!old->valid) {
			...
			// The frame that the host did the delta from
	...
	for(i = 0 ; i < PACKET_BACKUP ; i++) {
		...
		if(cl.snap.ps.commandTime >= cl.outPackets[ packetNum ].p_hostTime) {
...
int cl_connectedToPureHost;
int cl_connectedToCheatHost;
...
void CL_SystemInfoChanged(qboolean onlyGame) {
	...
	// NOTE TTimo:
	// when the hostId changes, any further messages we send to the host will use this new hostId
	// https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=475
	// in some cases, outdated cp commands might get sent with this news hostId
	cl.hostId = atoi(Info_ValueForKey(systemInfo, "sv_hostid"));
	...
	cl_connectedToPureHost = atoi(s);

	// parse/update fs_game in first place
	s = Info_ValueForKey(systemInfo, "fs_game");

	if(FS_InvalidGameDir(s)) {
		Com_Printf(S_COLOR_YELLOW "WARNING: Host sent invalid fs_game value %s\n", s);
	} else {
		Cvar_Set("fs_game", s);
	}

	// if game folder should not be set and it is set at the user side
	...
	cl_connectedToCheatHost = atoi(s);
	if(!cl_connectedToCheatHost) {
		Cvar_SetCheatState();
	}
	...
	if(com_sv_running->integer) {
		...
		// check pure host string
		s = Info_ValueForKey(systemInfo, "sv_paks");
		t = Info_ValueForKey(systemInfo, "sv_pakNames");
		FS_PureHostSetLoadedPaks(s, t);

		s = Info_ValueForKey(systemInfo, "sv_referencedPaks");
		t = Info_ValueForKey(systemInfo, "sv_referencedPakNames");
		FS_PureHostSetReferencedPaks(s, t);
	}
	...
	do {
		...
		// we don't really need any of these host cvars to be set on user-side
		if(!Q_stricmp(key, "sv_pure") || !Q_stricmp(key, "sv_hostid") || !Q_stricmp(key, "sv_fps")) {
			continue;
		}
		...
		if((cvar_flags = Cvar_Flags(key)) == CVAR_NONEXISTENT)
			Cvar_Get(key, value, CVAR_HOST_CREATED | CVAR_ROM);
		else
		{
			// If this cvar may not be modified by a host discard the value.
			if(!(cvar_flags &(CVAR_SYSTEMINFO | CVAR_HOST_CREATED | CVAR_USER_CREATED)))
			{
#ifndef STANDALONE
				if(Q_stricmp(key, "g_synchronousUsers") && Q_stricmp(key, "pmove_fixed") && Q_stricmp(key, "pmove_msec"))
#endif
				{
					Com_Printf(S_COLOR_YELLOW "WARNING: host is not allowed to set %s=%s\n", key, value);
					continue;
				}
			}
...
==================
*/
static void CL_ParseHostInfo(void)
{
	const char *hostInfo;
	size_t	len;

	hostInfo = cl.gameState.stringData
		+ cl.gameState.stringOffsets[ CS_HOSTINFO ];

	clc.sv_allowDownload = atoi(Info_ValueForKey(hostInfo,
		"sv_allowDownload"));
	Q_strncpyz(clc.sv_dlURL,
		Info_ValueForKey(hostInfo, "sv_dlURL"),
		sizeof(clc.sv_dlURL));
...
static void CL_ParseGamestate(msg_t *msg) {
	...
	// parse useful values out of CS_HOSTINFO
	CL_ParseHostInfo();
...
qboolean CL_ValidPakSignature(const byte *data, int len)
{
	...
	// but situation when host sends fragmented/shortened
...
A download message has been received from the host
=====================
*/
static void CL_ParseDownload(msg_t *msg) {
	...
	if(!*clc.downloadTempName) {
		Com_Printf("Host sending download, but no download was requested\n");
	...
	if(!size) { // A zero length block means EOF
		...
		// loading right away.  If we take a while to load, the host is happily trying
...
Command strings are just saved off until app asks for them
...
static void CL_ParseCommandString(msg_t *msg) {
	...
	if(cl_shownet->integer >= 3)
		Com_Printf(" %3i(%3i) %s\n", seq, clc.hostCommandSequence, s);

	// see if we have already executed stored it off
	if(clc.hostCommandSequence >= seq) {
		return;
	}
	clc.hostCommandSequence = seq;

	index = seq &(MAX_RELIABLE_COMMANDS-1);
	Q_strncpyz(clc.hostCommands[ index ], s, sizeof(clc.hostCommands[ index ]));
	clc.hostCommandsIgnore[ index ] = qfalse;
	...
	if((!cgvm && cls.state == CA_CONNECTED && clc.download != FS_INVALID_HANDLE) ||(cgvm && cls.state == CA_PRIMED)) {
		...
		if(!Q_stricmp(Cmd_Argv(0), "disconnect")) {
			text =(Cmd_Argc() > 1) ? va("Host disconnected: %s", Cmd_Argv(1)) : "Host disconnected.";
			Cvar_Set("com_errorMessage", text);
			Com_Printf("%s\n", text);
			if(!CL_Disconnect(qtrue)) { // restart user if not done already
				CL_FlushMemory();
			}
			return;
		}
...
void CL_ParseHostMessage(msg_t *msg) {
	...
	if(clc.reliableSequence - clc.reliableAcknowledge > MAX_RELIABLE_COMMANDS) {
		if(!clc.demoplaying) {
			Com_Printf(S_COLOR_YELLOW "WARNING: dropping %i commands from host\n", clc.reliableSequence - clc.reliableAcknowledge);
		}
	...
	while(1) {
		if(msg->readcount > msg->cursize) {
			Com_Error(ERR_DROP,"CL_ParseHostMessage: read past end of host message");
			break;
		}
		...
		switch(cmd) {
		default:
			Com_Error(ERR_DROP,"CL_ParseHostMessage: Illegible host message");
			break;
		case svc_nop:
			break;
		case svc_hostCommand:
