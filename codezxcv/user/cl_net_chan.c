...
static void CL_Netchan_Encode(msg_t *msg) {
	...
	string =(byte *)clc.hostCommands[ reliableAcknowledge &(MAX_RELIABLE_COMMANDS-1) ];
	index = 0;
	//
	key = clc.challenge ^ hostId ^ messageAcknowledge;
	for(i = CL_ENCODE_START; i < msg->cursize; i++) {
		// modify the key with the last received now acknowledged host command
		if(!string[index])
			index = 0;
...
static void CL_Netchan_Decode(msg_t *msg) {
	...
	index = 0;
	// xor the user challenge with the netchan sequence number(need something that changes every message)
	key = clc.challenge ^ LittleLong(*(unsigned *)msg->data);
	for(i = msg->readcount + CL_DECODE_START; i < msg->cursize; i++) {
		// modify the key with the last sent and with this message acknowledged user command
		if(!string[index])
			index = 0;
		if(string[index] > 127 || string[index] == '%') {
			key ^= '.' <<(i & 1);
...
void CL_Netchan_Transmit(netchan_t *chan, msg_t* msg) {
	...
	while(CL_Netchan_TransmitNextFragment(chan)) {
		// might happen if host die silently but user continue adding/sending commands
