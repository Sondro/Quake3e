 ...
 *  assumption that the X host will use the
...
//	Com_Printf("xtime: %p\n", xtime);
	...
	// some X hosts(like suse 8.1's) report weird event times
...
void HandleEvents(void)
{
	...
	while(XPending(dpy))
	{
		...
		case UserMessage:
			...
			if(event.xuser.data.l[0] == wmDeleteEvent) {
				Cmd_Clear();
				Com_Quit_f();
			}
			break;
...
** HGLRC, deleting the drawing context, and releasing the DC acquired
