...
void IN_UpdateWindow(RECT *window_rect, qboolean updateClipRegion)
{
	...
	if (GetUserRect(g_wv.hWnd, window_rect)) {
		POINT pos;
		int sx = 0, sy = 0;

		pos.x = window_rect->left;
		pos.y = window_rect->top;
		UserToScreen(g_wv.hWnd, &pos);
		...
		user_center.x = (window_rect->right - window_rect->left + sx) / 2;
		user_center.y = (window_rect->bottom - window_rect->top + sy) / 2;
		window_center = user_center;
		UserToScreen(g_wv.hWnd, &window_center);
		...
		user_center = window_center;
		ScreenToUser(g_wv.hWnd, &user_center);
	}
