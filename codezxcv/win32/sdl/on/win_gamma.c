...
static BOOL IsCurrentSessionRemoteable(void)
{
	...
	{
		#define TERMINAL_HOST_KEY TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Host\\")
		#define GLASS_SESSION_ID TEXT("GlassSessionId")

		HKEY hRegKey = NULL;
		LONG lResult;

		lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE,	TERMINAL_HOST_KEY, 0, KEY_READ, &hRegKey);
