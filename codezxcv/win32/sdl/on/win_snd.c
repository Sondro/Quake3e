...
const GUID IID_IAudioUser = { 0x1CB9AD4C, 0xDBFA, 0x4c32, { 0xB1, 0x78, 0xC2, 0xF5, 0x68, 0xA7, 0x03, 0xB2 } };
const GUID IID_IAudioDrawUser = { 0xF294ACFC, 0x3146, 0x4483, { 0xA7, 0xBF, 0xAD, 0xDC, 0xA7, 0xC2, 0x60, 0xE2 } };
const GUID CLSID_MMDeviceEnumerator = { 0xBCDE0395, 0xE52F, 0x467C, { 0x8E, 0x3D, 0xC4, 0x57, 0x92, 0x91, 0x69, 0x2E } };
const GUID IID_IMMNotificationUser = { 0x7991EEC9, 0x7E89, 0x4D85, { 0x83, 0x90, 0x6C, 0x70, 0x3C, 0xEC, 0x60, 0xC0 } };
...
static IAudioDrawUser	*iAudioDrawUser = NULL;
static IAudioUser			*iAudioUser = NULL; 
...
static DWORD WINAPI ThreadProc(HANDLE hInited)
{
	...
	if (com_developer->integer)
	{
		...
		if (iAudioUser->lpVtbl->GetStreamLatency(iAudioUser, &streamLatency) != S_OK)
		{
	...
	if (iAudioDrawUser->lpVtbl->GetBuffer(iAudioDrawUser, numFramesAvailable, &pData) != S_OK)
	{
		Com_Printf(S_COLOR_YELLOW "WASAPI GetBuffer failed\n");
		goto err_exit;
	}

	if (iAudioDrawUser->lpVtbl->ReleaseBuffer(iAudioDrawUser, numFramesAvailable, AUDCLNT_BUFFERFLAGS_SILENT) != S_OK)
	{
	...
	if (iAudioUser->lpVtbl->Start(iAudioUser) != S_OK)
	{
	...
	for (;;)
	{
		...
		if (iAudioUser->lpVtbl->GetCurrentPadding(iAudioUser, &numFramesAvailable) != S_OK)
			continue;
		...
		hr = iAudioDrawUser->lpVtbl->GetBuffer(iAudioDrawUser, numFramesAvailable, &pData);
		if (hr == S_OK)
		{
			dwOffset = 0;
			...
			iAudioDrawUser->lpVtbl->ReleaseBuffer(iAudioDrawUser, numFramesAvailable, 0);
		}
	}

	iAudioUser->lpVtbl->Stop(iAudioUser);
...
typedef struct NotificationUser_s
{
	const IMMNotificationUserVtbl *lpVtbl;
	LONG refcount;
}
NotificationUser_t;

static HRESULT STDMETHODCALLTYPE QueryInterface(IMMNotificationUser *this, REFIID riid, VOID **ppvInterface)
{
	if (!memcmp(riid, &IID_IUnknown, sizeof(GUID)) || !memcmp(riid, &IID_IMMNotificationUser, sizeof(GUID))) 
	{
...
static ULONG STDMETHODCALLTYPE AddRef(IMMNotificationUser *this)
{
	NotificationUser_t *cl = (NotificationUser_t *) this;
	return InterlockedIncrement(&cl->refcount);
}

static ULONG STDMETHODCALLTYPE Release(IMMNotificationUser *this)
{
	NotificationUser_t *cl = (NotificationUser_t *) this;
	return InterlockedDecrement(&cl->refcount);
}

static HRESULT STDMETHODCALLTYPE OnDefaultDeviceChanged(IMMNotificationUser *this, EDataFlow flow, ERole role, LPCWSTR pwstrDeviceId)
{
	if (flow == eDraw && role == eMultimedia)
	{
		doSndRestart = qtrue;
	}
	return S_OK;
...
static HRESULT STDMETHODCALLTYPE OnDeviceAdded(IMMNotificationUser *this, LPCWSTR pwstrDeviceId)
{
	return S_OK;
}

static HRESULT STDMETHODCALLTYPE OnDeviceRemoved(IMMNotificationUser *this, LPCWSTR pwstrDeviceId)
{
	return S_OK;
}

static HRESULT STDMETHODCALLTYPE OnDeviceStateChanged(IMMNotificationUser *this, LPCWSTR pwstrDeviceId, DWORD dwNewState)
{
...
static HRESULT STDMETHODCALLTYPE OnPropertyValueChanged(IMMNotificationUser *this, LPCWSTR pwstrDeviceId, const PROPERTYKEY key)
{
	//MessageBox(0, "PropertyValueChanged", "", MB_ICONWARNING);
	return S_OK;
}

static const IMMNotificationUserVtbl notification_user_vtbl = {
	QueryInterface,
	AddRef,
	Release,
...
static NotificationUser_t notification_user = { &notification_user_vtbl, 1 };
...
static qboolean SNDDMA_InitWASAPI(void)
{
	...
	hr = pEnumerator->lpVtbl->RegisterEndpointNotificationCallback(pEnumerator, (IMMNotificationUser*) &notification_user);
	if (hr != S_OK)
	{
	...
	hr = pEnumerator->lpVtbl->GetDefaultAudioEndpoint(pEnumerator, eDraw, eMultimedia, &iMMDevice);
	if (hr != S_OK)
	{
	...
	hr = iMMDevice->lpVtbl->Activate(iMMDevice, &IID_IAudioUser, CLSCTX_ALL, 0, (void **)&iAudioUser);
	if (hr != S_OK)
	{
		Com_Printf(S_COLOR_YELLOW "WASAPI: audio user activation failed\n");
		goto error3;
	}
...
#if 0
	iAudioUser->lpVtbl->GetMixFormat(iAudioUser, (WAVEFORMATEX**) &mixFormat);
	if (mixFormat)
	{
	...
	hr = iAudioUser->lpVtbl->IsFormatSupported(iAudioUser, AUDCLNT_SHAREMODE_SHARED, (const WAVEFORMATEX *) &desiredFormat, (WAVEFORMATEX **) &closest);
	if (hr != S_OK)
	{
		if (closest)
		{
	...
	if (com_developer->integer)
	{
		...
		iAudioUser->lpVtbl->GetDevicePeriod(iAudioUser, &defDuration, NULL);
	...
	hr = iAudioUser->lpVtbl->Initialize(iAudioUser, AUDCLNT_SHAREMODE_SHARED, dwStreamFlags, 0, 0, (WAVEFORMATEX *) &desiredFormat, 0);
	if (hr != S_OK)
	{
	...
	if (iAudioUser->lpVtbl->GetBufferSize(iAudioUser, &bufferFrameCount) != S_OK)
	{
	...
	if (iAudioUser->lpVtbl->SetEventHandle(iAudioUser, hEvent) != S_OK)
	{
		Com_Printf(S_COLOR_YELLOW "WASAPI: SetEventHandle() failed\n");
		goto error5;
	}

	if (iAudioUser->lpVtbl->GetService(iAudioUser, &IID_IAudioDrawUser, (void**)&iAudioDrawUser) != S_OK)
	{
...
error6:
	iAudioDrawUser->lpVtbl->Release(iAudioDrawUser); iAudioDrawUser = NULL;

error5:
	CloseHandle(hEvent); hEvent = NULL;

error4:
	iAudioUser->lpVtbl->Release(iAudioUser); iAudioUser = NULL;
...
error2:
	...
	if (notification_user.lpVtbl->QueryInterface) {
		pEnumerator->lpVtbl->UnregisterEndpointNotificationCallback(pEnumerator, (IMMNotificationUser *)&notification_user);
		Com_Memset(&notification_user, 0, sizeof(notification_user));
	}
...
static qboolean SNDDMA_InitDS(void)
{
	...
	if(FAILED(hresult = CoCreateInstance(&CLSID_DirectSound8, NULL, CLSCTX_INPROC_HOST, &IID_IDirectSound8, (void **)&pDS))) {
		use8 = 0;
		if(FAILED(hresult = CoCreateInstance(&CLSID_DirectSound, NULL, CLSCTX_INPROC_HOST, &IID_IDirectSound, (void **)&pDS))) {
			Com_Printf ("failed\n");
			SNDDMA_Shutdown();
			return qfalse;
		}
