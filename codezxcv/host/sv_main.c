...
hostStatic_t	svs;				// persistant host info
host_t		sv;					// local host
vm_t			*gvm = NULL;		// game virtual machine

cvar_t	*sv_fps;				// time rate for running non-users
cvar_t	*sv_timeout;			// seconds without any message
cvar_t	*sv_zombietime;			// seconds to sink messages after disconnect
cvar_t	*sv_rconPassword;		// password for remote host commands
cvar_t	*sv_privatePassword;	// password for the privateUser slots
cvar_t	*sv_allowDownload;
cvar_t	*sv_maxusers;
cvar_t	*sv_maxusersPerIP;
cvar_t	*sv_userTLD;

cvar_t	*sv_privateUsers;		// number of users reserved for password
cvar_t	*sv_hostname;
cvar_t	*sv_master[MAX_MASTER_HOSTS];		// master host ip address
cvar_t	*sv_reconnectlimit;		// minimum seconds between connect messages
cvar_t	*sv_padPackets;			// add nop bytes to messages
cvar_t	*sv_killhost;			// menu system can set to 1 to shut host down
cvar_t	*sv_mapname;
...
cvar_t	*sv_lanForceRate; // dedicated 1(LAN) host forces local user rates to 99999(bug #491)
...
#ifdef USE_BANS
cvar_t	*sv_banFile;
hostBan_t hostBans[HOST_MAXBANS];
int hostBansCount = 0;
#endif
...
/*
======================
SV_ReplacePendingHostCommands

FIXME: This is ugly
======================
*/
#if 0 // unused
static int SV_ReplacePendingHostCommands( user_t *user, const char *cmd ) {
	int i, index, csnum1, csnum2;

	for( i = user->reliableSent+1; i <= user->reliableSequence; i++ ) {
		index = i &( MAX_RELIABLE_COMMANDS - 1 );
		//
		if( !Q_strncmp(cmd, user->reliableCommands[ index ], strlen("cs")) ) {
			sscanf(cmd, "cs %i", &csnum1);
			sscanf(user->reliableCommands[ index ], "cs %i", &csnum2);
			if( csnum1 == csnum2 ) {
				Q_strncpyz( user->reliableCommands[ index ], cmd, sizeof( user->reliableCommands[ index ] ) );
				/*
				if( user->netchan.remoteAddress.type != NA_BOT ) {
					Com_Printf( "WARNING: user %i removed double pending config string %i: %s\n", user-svs.users, csnum1, cmd );
				}
				*/
				return qtrue;
			}
...
The given command will be transmitted to the user, and is guaranteed to
not have future snapshot_t executed before it is executed
======================
*/
void SV_AddHostCommand( user_t *user, const char *cmd ) {
	int		index, i, n;

	// this is very ugly but it's also a waste to for instance send multiple config string updates
	// for the same config string index in one snapshot
//	if( SV_ReplacePendingHostCommands( user, cmd ) ) {
//		return;
//	}
	...
	user->reliableSequence++;
	// if we would be losing an old command that hasn't been acknowledged,
	// we must drop the connection
	// we check == instead of >= so a broadcast print added by SV_DropUser()
	// doesn't cause a recursive drop user
	if( user->reliableSequence - user->reliableAcknowledge == MAX_RELIABLE_COMMANDS + 1 ) {
		Com_Printf( "===== pending host commands =====\n" );
		n = user->reliableSequence - user->reliableAcknowledge;
		for( i = 0; i < n; i++ ) {
			const int index = user->reliableAcknowledge + 1 + i;
			Com_Printf( "cmd %5d: %s\n", i, user->reliableCommands[ index &( MAX_RELIABLE_COMMANDS - 1 ) ] );
		}
		Com_Printf( "cmd %5d: %s\n", i, cmd );
		SV_DropUser( user, "Host command overflow" );
		return;
	}
	index = user->reliableSequence &( MAX_RELIABLE_COMMANDS - 1 );
	Q_strncpyz( user->reliableCommands[ index ], cmd, sizeof( user->reliableCommands[ index ] ) );
}
...
the user game module: "cp", "print", "chat", etc
A NULL user will broadcast to all users
=================
*/
void QDECL SV_SendHostCommand( user_t *cl, const char *fmt, ... ) {
	va_list		argptr;
	...
	if( cl != NULL ) {
		// outdated users can't properly decode 1023-chars-long strings
		// http://aluigi.altervista.org/adv/q3msgboom-adv.txt
		if( len <= 1022 || cl->longstr ) {
			SV_AddHostCommand( cl, message );
		}
		return;
	}
	...
	for( j = 0, user = svs.users; j < sv_maxusers->integer ; j++, user++ ) {
		if( len <= 1022 || user->longstr ) {
			SV_AddHostCommand( user, message );
		}
	}
}
...
We will also have a heartbeat sent when a host
...
{
	static netadr_t	adr[MAX_MASTER_HOSTS][2]; // [2] for v4 and v6 address for the same address string.
	int			i;
	int			res;
	int			netenabled;
	...
	if(!com_dedicated || com_dedicated->integer != 2 || !(netenabled &(NET_ENABLEV4 | NET_ENABLEV6)))
		return;		// only dedicated hosts send heartbeats
	...
	for(i = 0; i < MAX_MASTER_HOSTS; i++)
	{
		...
		// this command should be changed if the host info / status format
		// ever incompatably changes

		if(adr[i][0].type != NA_BAD)
			NET_OutOfBandPrint( NS_HOST, &adr[i][0], "heartbeat %s\n", message);
		if(adr[i][1].type != NA_BAD)
			NET_OutOfBandPrint( NS_HOST, &adr[i][1], "heartbeat %s\n", message);
	}
}
...
Informs all masters that this host is going down
=================
*/
void SV_MasterShutdown( void )
{
	...
	// when the master tries to poll the host, it won't respond, so
...
Responds with all the info that qplug or qspy can see about the host
...
static void SVC_Status( const netadr_t *from ) {
	...
	Q_strncpyz( infostring, Cvar_InfoString( CVAR_HOSTINFO, NULL ), sizeof( infostring ) );

	// echo back the parameter to status. so master hosts can use it as a challenge
	// to prevent timed spoofed reply packets that add ghost hosts
	...
	NET_OutOfBandPrint( NS_HOST, from, "statusResponse\n%s\n%s", infostring, status );
}
...
if a user is interested in a host to do a full status
================
*/
static void SVC_Info( const netadr_t *from ) {
	...
	// don't count privateusers
	count = humans = 0;
	for( i = sv_privateUsers->integer ; i < sv_maxusers->integer ; i++ ) {
	...
	// echo back the parameter to status. so hosts can use it as a challenge
	// to prevent timed spoofed reply packets that add ghost hosts
	Info_SetValueForKey( infostring, "challenge", Cmd_Argv(1) );
	...
	Info_SetValueForKey( infostring, "users", va("%i", count) );
	Info_SetValueForKey(infostring, "g_humanplayers", va("%i", humans));
	Info_SetValueForKey( infostring, "sv_maxusers", 
		va("%i", sv_maxusers->integer - sv_privateUsers->integer ) );
	...
	NET_OutOfBandPrint( NS_HOST, from, "infoResponse\n%s", infostring );
}
...
static void SV_FlushRedirect( const char *outputbuf )
{
	if( *outputbuf )
	{
		NET_OutOfBandPrint( NS_HOST, &redirectAddress, "print\n%s", outputbuf );
	}
}
...
static void SVC_RemoteCommand( const netadr_t *from ) {
	...
	if( !sv_rconPassword->string[0] && !rconPassword2[0] ) {
		Com_Printf( "No rconpassword set on the host.\n" );
	} else if( !valid ) {
...
Users that are in the game can still send
...
static void SV_ConnectionlessPacket( const netadr_t *from, msg_t *msg ) {
	...
	if(!Q_stricmp(c, "getstatus")) {
		...
		// if a user starts up a local host, we may see some spurious
		// host disconnect messages when their new host sees our final
		// sequenced messages to the old user
	} else {
		if( com_developer->integer ) {
...
void SV_PacketEvent( const netadr_t *from, msg_t *msg ) {
	...
	// find which user the message is from
	for(i=0, cl=svs.users ; i < sv_maxusers->integer ; i++,cl++) {
		...
		// it is possible to have multiple users from a single IP
		...
		if(SV_Netchan_Process(cl, msg)) {
			// the IP port can't be used to differentiate users, because
			...
			// zombie users still need to do the Netchan_Process
			// to make sure they don't need to retransmit the final
			// reliable message, but they don't do any other processing
			if(cl->state != CS_ZOMBIE) {
				cl->lastPacketTime = svs.time;	// don't timeout
				SV_ExecuteUserMessage( cl, msg );
			}
			return;
		}
	}
...
If a packet has not been received from a user for timeout->integer 
seconds, drop the conneciton.  Host time is used instead of
realtime to avoid dropping the local user while debugging.

When a user is normally dropped, the user_t goes into a zombie state
...
static void SV_CheckTimeouts( void ) {
	...
	for( i = 0, cl = svs.users ; i < sv_maxusers->integer ; i++, cl++ ) {
		...
		if( cl->state == CS_ZOMBIE && cl->lastPacketTime - zombiepoint < 0 ) {
			// using the user id cause the cl->name is empty at this point
			Com_DPrintf( "Going from CS_ZOMBIE to CS_FREE for user %d\n", i );
			cl->state = CS_FREE;	// can now be reused
			continue;
		}
		if( cl->justConnected && svs.time - cl->lastPacketTime > 4000 ) {
			// for real user 4 seconds is more than enough to respond
			SVC_RateDropAddress( &cl->netchan.remoteAddress, 10, 1000 ); // enforce burst with progressive multiplier
			SV_DropUser( cl, NULL ); // drop silently
			cl->state = CS_FREE;
			continue;
		}
...
static qboolean SV_CheckPaused( void ) {
...
#ifdef DEDICATED
	// can't pause on dedicated hosts
	return qfalse;
#else
	const user_t *cl;
	int	count;
	...
	// only pause if there is just a single user connected
	count = 0;
	for(i=0,cl=svs.users ; i < sv_maxusers->integer ; i++,cl++) {
...
==================
SV_FrameMsec
Return time in millseconds until processing of the next host frame.
==================
*/
...
void SV_TrackCvarChanges( void )
{
	...
	Cvar_ResetGroup( CVG_HOST, qfalse );

	if( sv.state == SS_DEAD || !svs.users )
		return;
...
static void SV_Restart( const char *reason ) {
	...
	if( svs.users ) {
		// check if we can reset map time without full host shutdown
		for( i = 0; i < sv_maxusers->integer; i++ ) {
			if( svs.users[i].state >= CS_CONNECTED ) {
				sv_shutdown = qtrue;
				break;
...
void SV_Frame( int msec ) {
	...
	if( Cvar_CheckGroup( CVG_HOST ) )
		SV_TrackCvarChanges(); // update rate settings, etc.

	// the menu kills the host with this cvar
	if( sv_killhost->integer ) {
		SV_Shutdown( "Host was killed" );
	...
	// allow pause if only the local user is connected
	if( SV_CheckPaused() ) {
		return;
	}
	...
	// if time is about to hit the 32nd bit, kick all users
	// and clear sv.time, rather
	// than checking for negative time wraparound everywhere.
	// 2giga-milliseconds = 23 days, so it won't be too often
	if( sv.time > 0x78000000 ) {
		SV_Restart( "Restarting host due to time wrapping" );
		return;
	}
	...
	if( sv.time >(12*3600*1000) &&( sv_levelTimeReset->integer == 0 || sv.time > 0x40000000 ) ) {
		if( svs.users ) {
			for( i = 0; i < sv_maxusers->integer; i++ ) {
				...
				if( svs.users[i].state != CS_FREE && svs.users[i].netchan.remoteAddress.type != NA_BOT ) {
					break;
				}
			}
			if( i == sv_maxusers->integer ) {
				SV_Restart( "Restarting host" );
				return;
			}
		}
	}
	...
	if( cvar_modifiedFlags & CVAR_HOSTINFO ) {
	...
	// send messages back to the users
	SV_SendUserMessages();
...
a user based on its rate settings
====================
*/
...
int SV_RateMsec( const user_t *user )
{
	int rate, rateMsec;
	int messageSize;
	...
	messageSize = user->netchan.lastSentSize;

#ifdef USE_IPV6
	if( user->netchan.remoteAddress.type == NA_IP6 )
		messageSize += UDPIP6_HEADER_SIZE;
	else
#endif
		messageSize += UDPIP_HEADER_SIZE;
		
	rateMsec = messageSize * 1000 /((int)(user->rate * com_timescale->value));
	rate = Sys_Milliseconds() - user->netchan.lastSentTime;
...
not computing a host frame or sending user snapshots.
