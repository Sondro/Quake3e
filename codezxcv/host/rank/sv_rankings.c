...
void SV_RankBegin( char *gamekey )
{
	...
	// only allow official game key on pure hosts
...
*/
	...
	s_ranked_players = Z_Malloc( sv_maxusers->value * 
		sizeof(ranked_player_t) );
	memset((void*)s_ranked_players, 0 ,sv_maxusers->value 
		* sizeof(ranked_player_t));
}
...
grank_status_t SV_RankUserStatus( int index )
{
	...
	assert( s_ranked_players != NULL );
	assert( index >= 0 );
	assert( index < sv_maxusers->value );
...
qboolean SV_RankUserValidate( int index, const char* player_id, const char* key, int token_len, int rank, char* name )
{
	...
	if((player_id != NULL) &&(key != NULL))
	{
		...
		// can be shared by both host side login and user side login
		
		// for user side logined in players
		// host is creating GR_OPT_PLAYERCONTEXT
		init = GRankInit( 0, SV_RankGameKey, GR_OPT_POLL, GR_OPT_END );
	...
	{
		// make host side login(bots) happy
		status = GR_STATUS_OK;
	}
	...
	qboolean accum )
{
	...
	assert( index1 >= -1 );
	assert( index1 < sv_maxusers->value );
	assert( index2 >= -1 );
	assert( index2 < sv_maxusers->value );
	assert( s_ranked_players );
...
static void SV_RankNewGameCBF( GR_NEWGAME* gr_newgame, void* cbf_arg )
{
	...
	if( gr_newgame->status == GR_STATUS_OK )
	{
		...
		// set CS_GRANK rankingsGameID to pass to user
		...
		// start new match
		match = GRankStartMatch( s_host_context );
		s_host_match = match.match;
...
static void SV_RankSendReportsCBF( GR_STATUS* status, void* cbf_arg )
{
	...
	assert( status != NULL );
	// NULL cbf_arg means host is sending match reports
	...
	{
		Com_DPrintf( "SV_RankSendReportsCBF: host\n" );
		context = s_host_context;
	}
	else
	{
...
static void SV_RankCleanupCBF( GR_STATUS* status, void* cbf_arg )
{
	...
	assert( status != NULL );
	// NULL cbf_arg means host is cleaning up
