...
==================
*/
user_t *SV_GetPlayerByHandle( void ) {
	user_t	*cl;
	int			i;
	char		*s;
	...
	if(!s[i])
	{
		...
		if(plid >= 0 && plid < sv_maxusers->integer)
		{
			cl = &svs.users[plid];
	...
	Com_Printf( "Player %s is not on the host\n", s );
...
==================
*/
static user_t *SV_GetPlayerByNum( void ) {
	user_t	*cl;
	int			i;
	int			idnum;
	char		*s;
	...
	idnum = atoi( s );
	if( idnum < 0 || idnum >= sv_maxusers->integer ) {
		Com_Printf( "Bad user slot: %i\n", idnum );
		return NULL;
	}

	cl = &svs.users[idnum];
	if( !cl->state ) {
		Com_Printf( "User %i is not active\n", idnum );
		return NULL;
	}
	return cl;
}
...
Restart the host on a different map
==================
*/
static void SV_Map_f( void ) {
	...
	// a typo at the host console won't end the game
	...
	Cvar_Get("g_gametype", "0", CVAR_HOSTINFO | CVAR_USERINFO | CVAR_LATCH );

	cmd = Cmd_Argv(0);
	if( Q_stricmpn( cmd, "sp", 2 ) == 0 ) {
		Cvar_SetIntegerValue( "g_gametype", GT_SINGLE_PLAYER );
		Cvar_Set( "g_doWarmup", "0" );
		// may not set sv_maxusers directly, always set latched
		Cvar_SetLatched( "sv_maxusers", "8" );
		cmd += 2;
...
Completely restarts a level, but doesn't send a new gamestate to the users.
...
static void SV_MapRestart_f( void ) {
	...
	if( com_frameTime == sv.hostId ) {
		return;
	}

	// make sure host is running
	if( !com_sv_running->integer ) {
		Com_Printf( "Host is not running.\n" );
		return;
	}
	...
	if( sv_maxusers->modified || sv_gametype->modified || sv_pure->modified ) {
		char	mapname[MAX_QPATH];
	...
	// TTimo - don't update restartedhostId there, otherwise we won't deal correctly with multiple map_restart
	sv.hostId = com_frameTime;
	Cvar_Set( "sv_hostid", va("%i", sv.hostId ) );

	// if a map_restart occurs while a user is changing maps, we need
	// to give them the correct time so that when they finish loading
	// they don't violate the backwards time check in cl_app.c
	for(i=0 ; i<sv_maxusers->integer ; i++) {
		if(svs.users[i].state == CS_PRIMED) {
			svs.users[i].oldHostTime = sv.restartTime;
		}
	}
	...
	// connect and begin all the users
	for( i = 0 ; i < sv_maxusers->integer ; i++ ) {
		user = &svs.users[i];

		// send the new gamestate to all connected users
		if( user->state < CS_CONNECTED ) {
			continue;
		}
		...
		SV_AddHostCommand( user, "map_restart\n" );

		// connect the user again, without the firstTime flag
		denied = GVM_ArgPtr( VM_Call( gvm, 3, GAME_USER_CONNECT, i, qfalse, isBot ) );
		if( denied ) {
			// this generally shouldn't happen, because the user
			// was connected before the level change
			SV_DropUser( user, denied );
			Com_Printf( "SV_MapRestart_f(%d): dropped user %i - denied!\n", delay, i );
			continue;
		}

		if( user->state == CS_ACTIVE )
			SV_UserEnterWorld( user, &user->lastUsercmd );
		else {
			// If we don't reset user->lastUsercmd and are restarting during map load,
			// the user will hang because we'll use the last Usercmd from the previous map,
			// which is wrong obviously.
			SV_UserEnterWorld( user, NULL );
		}
	}
...
Kick a user off of the host  FIXME: move to game
==================
*/
...
#ifndef STANDALONE
// these functions require the auth host which of course is not available anymore for stand-alone games.
...
Ban a user from being able to play on this host through the auth
host
==================
*/
static void SV_Ban_f( void ) {
	...
	// look up the authorize host's IP
	if( !svs.authorizeAddress.ip[0] && svs.authorizeAddress.type != NA_BAD ) {
		Com_Printf( "Resolving %s\n", AUTHORIZE_HOST_NAME );
		if( !NET_StringToAdr( AUTHORIZE_HOST_NAME, &svs.authorizeAddress, NA_IP ) ) {
	...
	// otherwise send their ip to the authorize host
	if( svs.authorizeAddress.type != NA_BAD ) {
		NET_OutOfBandPrint( NS_HOST, &svs.authorizeAddress,
...
static void SV_BanNum_f( void ) {
	...
	if( Cmd_Argc() != 2 ) {
		Com_Printf("Usage: banUser <user number>\n");
		return;
	}
...
static void SV_RehashBans_f(void)
{
	...
	if((filelen = FS_SV_FOpenFileRead(filepath, &readfrom)) >= 0)
	{
		...
		for(index = 0; index < HOST_MAXBANS && curpos + 2 < endpos; index++)
		{
			...
			*newlinepos = '\0';
			...
			if(NET_StringToAdr(curpos + 2, &hostBans[index].ip, NA_UNSPEC))
			{
				hostBans[index].isexception =(curpos[0] != '0');
				hostBans[index].subnet = atoi(maskpos);
				
				if(hostBans[index].ip.type == NA_IP &&
				  (hostBans[index].subnet < 1 || hostBans[index].subnet > 32))
				{
					hostBans[index].subnet = 32;
				}
				else if(hostBans[index].ip.type == NA_IP6 &&
					(hostBans[index].subnet < 1 || hostBans[index].subnet > 128))
				{
					hostBans[index].subnet = 128;
				}
			}
...
{
	if(index == hostBansCount - 1)
		hostBansCount--;
	else if(index < ARRAY_LEN(hostBans) - 1)
	{
		memmove(hostBans + index, hostBans + index + 1,(hostBansCount - index - 1) * sizeof(*hostBans));
		hostBansCount--;
	}
	else
		return qtrue;
...
Ban a user from being able to play on this host based on his ip address.
==================
*/
...
static void SV_AddBanToList(qboolean isexception)
{
	...
	if(argc < 2 || argc > 3)
	{
		Com_Printf("Usage: %s(ip[/subnet] | usernum [subnet])\n", Cmd_Argv(0));
		return;
	}

	if(hostBansCount >= ARRAY_LEN(hostBans))
	{
	...
	{
		// This is an ip address, not a user num.
	...
	hostBans[hostBansCount].ip = ip;
	hostBans[hostBansCount].subnet = mask;
	hostBans[hostBansCount].isexception = isexception;
...
static void SV_DelBanFromList(qboolean isexception)
{
	...
	else
	{
		...
		if(todel < 1 || todel > hostBansCount)
		{
		...
		for(index = 0; index < hostBansCount; index++)
		{
			if(hostBans[index].isexception == isexception)
			{
				count++;
				...
				if(count == todel)
				{
					Com_Printf("Deleting %s %s/%d\n",
					   isexception ? "exception" : "ban",
					   NET_AdrToString(&hostBans[index].ip), hostBans[index].subnet);
...
static void SV_ListBans_f(void)
{
	...
	// List all bans
	for(index = count = 0; index < hostBansCount; index++)
	{
		ban = &hostBans[index];
		if(!ban->isexception)
		{
...
static void SV_KickNum_f( void ) {
	...
	if( Cmd_Argc() != 2 ) {
		Com_Printf("Usage: kicknum <user number>\n");
		return;
	}
	...
	if( cl->netchan.remoteAddress.type == NA_LOOPBACK ) {
		SV_SendHostCommand(NULL, "print \"%s\"", "Cannot kick host player\n");
		return;
	}
...
static void SV_Status_f( void ) {
	...
	int max_namelength;
	int max_addrlength;
	char names[ MAX_USERS * MAX_NAME_LENGTH ], *np[ MAX_USERS ], nl[ MAX_USERS ], *nc;
	char addrs[ MAX_USERS * 48 ], *ap[ MAX_USERS ], al[ MAX_USERS ], *ac;
...
static void SV_ConSay_f( void ) {
	...
	SV_SendHostCommand( NULL, "chat \"%s\"", text );
}
...
static void SV_ConTell_f( void ) {
	...
	if( Cmd_Argc() < 3 ) {
		Com_Printf( "Usage: tell <user number> <text>\n" );
		return;
	}
	...
	Com_Printf( "%s\n", text );
	SV_SendHostCommand( cl, "chat \"%s\"", text );
}
...
Also called by SV_DropUser, SV_DirectConnect, and SV_SpawnHost
==================
*/
...
Examine the hostinfo string
===========
*/
static void SV_Hostinfo_f( void ) {
	const char *info;
	...
	info = sv.configstrings[ CS_HOSTINFO ];
	if( info ) {
		Info_Print( info );
	}
}
...
=================
*/
static void SV_KillHost_f( void ) {
	SV_Shutdown( "killhost" );
}
...
static void SV_Locations_f( void ) {
	...
	if( !sv_userTLD->integer ) {
		Com_Printf( "Disabled on this host.\n" );
		return;
	}
...
void SV_AddOperatorCommands( void ) {
	...
	if(!Cvar_VariableIntegerValue("com_standalone"))
	{
		...
		Cmd_AddCommand("banUser", SV_BanNum_f);
	}
#endif
#endif
	Cmd_AddCommand("userkick", SV_KickNum_f);
...
#endif
	Cmd_AddCommand("killhost", SV_KillHost_f);
#ifdef USE_BANS	
...
#if 0
	// removing these won't let the host start again
	Cmd_RemoveCommand("heartbeat");
	Cmd_RemoveCommand("kick");
	Cmd_RemoveCommand("userkick");
...
{
	Cmd_AddCommand( "hostinfo", SV_Hostinfo_f );
