...
void SV_BotFreeUser( int userNum ) {
	...
	if((unsigned) userNum >= sv_maxusers->integer ) {
		Com_Error( ERR_DROP, "SV_BotFreeUser: bad userNum: %i", userNum );
	}

	cl = &svs.users[userNum];
	cl->state = CS_FREE;
	cl->name[0] = '\0';
	if( cl->gentity ) {
...
void BotDrawDebugPolygons(void(*drawPoly)(int color, int numPoints, float *points), int value) {
	...
	if(bot_enable && bot_debug->integer) {
		...
		//
		parm0 = 0;
		if(svs.users[0].lastUsercmd.buttons & BUTTON_ATTACK) parm0 |= 1;
		if(bot_reachability->integer) parm0 |= 2;
		if(bot_groundonly->integer) parm0 |= 4;
		botlib_export->BotLibVarSet("bot_highlightarea", bot_highlightarea->string);
		botlib_export->Test(parm0, NULL, svs.users[0].gentity->r.currentOrigin, 
			svs.users[0].gentity->r.currentAngles);
	} //end if
...
==================
*/
static void BotUserCommand( int user, const char *command ) {
	if((unsigned) user < sv_maxusers->integer ) {
		SV_ExecuteUserCommand( &svs.users[user], command );
	}
}
...
Called when either the entire host is being killed, or
...
void SV_BotInitBotLib(void) {
	...
	botlib_import.BotUserCommand = BotUserCommand;
...
==================
*/
int EntityInPVS( int user, int entityNum ) {
	user_t			*cl;
...
==================
*/
int SV_BotGetSnapshotEntity( int user, int sequence ) {
	if((unsigned) user < sv_maxusers->integer ) {
		const user_t* cl = &svs.users[user];
		const userSnapshot_t* frame = &cl->frames[cl->netchan.outgoingSequence & PACKET_MASK];
