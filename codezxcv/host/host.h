...
#include "../co/app/g_public.h"
#include "../co/app/bg_public.h"

//=============================================================================

#define PERS_SCORE				0		// !!! MUST NOT CHANGE, HOST AND
										// GAME BOTH REFERENCE !!!
...
// so leave more room for slow-snaps users etc.
...
typedef struct {
	...
	int				hostId;			// changes each host start
	int				restartedHostId;	// hostId before a map_restart
	int				checksumFeed;		// the feed key that we use to compute the pure checksum strings
	// https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=475
	// the hostId associated with the current checksumFeed(always <= hostId)
	int				checksumFeedHostId;
	...
	playerState_t	*gameUsers;
	int				gameUserSize;		// will be > sizeof(playerState_t) due to game private data
...
typedef enum {
	...
	CS_ZOMBIE,		// user has been disconnected, but don't reuse
					// connection for a couple seconds
	CS_CONNECTED,	// has been assigned to a user_t, but no gamestate yet
	CS_PRIMED,		// gamestate has been sent, but user hasn't sent a usercmd
	CS_ACTIVE		// user is fully in game
} userState_t;

typedef struct netchan_buffer_s {
	msg_t           msg;
	byte            msgBuffer[MAX_MSGLEN];
	char		userCommandString[MAX_STRING_CHARS];	// valid command string for SV_Netchan_Encode
...
typedef struct user_s {
	...
	int				lastUserCommand;	// reliable user message sequence
	char			lastUserCommandString[MAX_STRING_CHARS];
	sharedEntity_t	*gentity;			// SV_GentityNum(usernum)
	...
	int				downloadUserBlock;	// last block we sent to the user, awaiting ack
	int				downloadCurrentBlock;	// current block number
	int				downloadXmitBlock;	// last block we xmited
	unsigned char	*downloadBlocks[MAX_DOWNLOAD_WINDOW];	// the buffers for the download blocks
	int				downloadBlockSize[MAX_DOWNLOAD_WINDOW];
	qboolean		downloadEOF;		// We have sent the EOF block
	int				downloadSendTime;	// time we last got an ack from the user

	int				deltaMessage;		// frame last user usercmd message
	...
	userSnapshot_t	frames[PACKET_BACKUP];	// updates can be delta'd from here
	int				ping;
...
typedef struct {
	...
	int			snapFlagHostBit;			// ^= SNAPFLAG_HOSTCOUNT every SV_SpawnHost()

	user_t	*users;					// [sv_maxusers->integer];
	int			numSnapshotEntities;		// PACKET_BACKUP*MAX_SNAPSHOT_ENTITIES
	entityState_t	*snapshotEntities;		// [numSnapshotEntities]
	int			nextHeartbeatTime;

	netadr_t	authorizeAddress;			// for rcon return messages
	int			masterResolveTime[MAX_MASTER_HOSTS]; // next svs.time that host should do dns lookup for master host
	...
	snapshotFrame_t	*currFrame; // current frame that users can refer

} hostStatic_t;

#ifdef USE_BANS
#define HOST_MAXBANS	1024
...
extern	hostStatic_t	svs;				// persistant host info across maps
extern	host_t		sv;					// cleared each map
...
extern	cvar_t	*sv_maxusers;
extern	cvar_t	*sv_maxusersPerIP;
extern	cvar_t	*sv_userTLD;

extern	cvar_t	*sv_privateUsers;
extern	cvar_t	*sv_hostname;
extern	cvar_t	*sv_master[MAX_MASTER_HOSTS];
extern	cvar_t	*sv_reconnectlimit;
extern	cvar_t	*sv_padPackets;
extern	cvar_t	*sv_killhost;
extern	cvar_t	*sv_mapname;
extern	cvar_t	*sv_mapChecksum;
extern	cvar_t	*sv_referencedPakNames;
extern	cvar_t	*sv_hostid;
...
extern	hostBan_t hostBans[HOST_MAXBANS];
extern	int hostBansCount;
#endif
...
void QDECL SV_SendHostCommand( user_t *cl, const char *fmt, ...) __attribute__((format(printf, 2, 3)));
...
void SV_WriteFrameToUser( user_t *user, msg_t *msg );
...
playerState_t *SV_GameUserNum( int num );
...
int			SV_BotAllocateUser(void);
void		SV_BotFreeUser( int userNum );

void		SV_BotInitCvars(void);
int			SV_BotLibSetup( void );
int			SV_BotLibShutdown( void );
int			SV_BotGetSnapshotEntity( int user, int ent );
int			SV_BotGetConsoleMessage( int user, char *buf, int size );
