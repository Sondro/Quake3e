...
// the game allocates gentities with private information after the host shared part
...
playerState_t *SV_GameUserNum( int num ) {
	...
	ps =(playerState_t *)((byte *)sv.gameUsers + sv.gameUserSize*(num));
...
/*
===============
SV_GameSendHostCommand

Sends a command string to a user
===============
*/
static void SV_GameSendHostCommand( int userNum, const char *text ) {
	if( userNum == -1 ) {
		SV_SendHostCommand( NULL, "%s", text );
	} else {
		if( userNum < 0 || userNum >= sv_maxusers->integer ) {
			return;
		}
		SV_SendHostCommand( svs.users + userNum, "%s", text );	
	}
}
...
Disconnects the user with a message
===============
*/
static void SV_GameDropUser( int userNum, const char *reason ) {
	if( userNum < 0 || userNum >= sv_maxusers->integer ) {
		return;
	}
	SV_DropUser( svs.users + userNum, reason );	
}
...
===============
*/
static void SV_GetHostinfo( char *buffer, int bufferSize ) {

	if( bufferSize < 1 ) {
		Com_Error( ERR_DROP, "SV_GetHostinfo: bufferSize == %i", bufferSize );
	}
	if( sv.state != SS_GAME || !sv.configstrings[ CS_HOSTINFO ] ) {
		Q_strncpyz( buffer, Cvar_InfoString( CVAR_HOSTINFO, NULL ), bufferSize );
	} else {
		Q_strncpyz( buffer, sv.configstrings[ CS_HOSTINFO ], bufferSize );
	}
}
...
===============
*/
static void SV_LocateGameData( sharedEntity_t *gEnts, int numGEntities, int sizeofGEntity_t, playerState_t *users, int sizeofGameUser ) {
	...
	if( !gvm->entryPoint ) {
		...
		if( sizeofGameUser > gvm->exactDataLength / MAX_USERS ) {
			Com_Error( ERR_DROP, "%s: bad game user size %i", __func__, sizeofGameUser );	
		} else if((byte*)users +(sizeofGameUser * MAX_USERS) > gvm->dataBase + gvm->exactDataLength ) {
			Com_Error( ERR_DROP, "%s: users located out of data segment", __func__ );
		}
	}
	...
	sv.gameUsers = users;
	sv.gameUserSize = sizeofGameUser;
}
...
===============
*/
static void SV_GetUsercmd( int userNum, usercmd_t *cmd ) {
	if((unsigned) userNum < sv_maxusers->integer ) {
		*cmd = svs.users[ userNum ].lastUsercmd;
	} else {
		Com_Error( ERR_DROP, "%s(): bad userNum: %i", __func__, userNum );
	}
}
...
static intptr_t SV_GameSystemCalls( intptr_t *args ) {
	...
	case G_LOCATE_GAME_DATA:
		...
		return 0;
	case G_DROP_USER:
		SV_GameDropUser( args[1], VMA(2) );
		return 0;
	case G_SEND_HOST_COMMAND:
		SV_GameSendHostCommand( args[1], VMA(2) );
		return 0;
	case G_LINKENTITY:
		SV_LinkEntity( VMA(1) );
		return 0;
	...
	case G_BOT_ALLOCATE_USER:
		return SV_BotAllocateUser();
	case G_BOT_FREE_USER:
		SV_BotFreeUser( args[1] );
		return 0;
	...
	case BOTLIB_USER_COMMAND:
		{
			unsigned userNum = args[1];
			if( userNum < sv_maxusers->integer )
			{
				SV_UserThink( &svs.users[ userNum ], VMA(2) );
			}
		}
		return 0;
	...
	case BOTLIB_AAS_PREDICT_USER_MOVEMENT:
		return botlib_export->aas.AAS_PredictUserMovement( VMA(1), args[2], VMA(3), args[4], args[5],
