...
int AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
	...
	teststart[2] += 64;
	trace = AAS_TraceUserBBox(teststart, origin, PRESENCE_CROUCH, -1);
	if (trace.startsolid)
	{
...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		areas = AAS_LinkEntityUserBBox(absmins, absmaxs, -1, PRESENCE_CROUCH);
		for (link = areas; link; link = link->next_area)
		{
		...
		Com_Memset(&move, 0, sizeof(aas_usermove_t));
		AAS_UserMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL, qfalse,
...
int AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs, vec3_t goalorigin)
{
	...
	if (areanum)
	{
		//drop user bbox down and try again
		VectorCopy(start, end);
		start[2] += 0.25;
		end[2] -= 50;
		trace = AAS_TraceUserBBox(start, end, PRESENCE_CROUCH, -1);
		if (!trace.startsolid)
		{
		...
		else
		{
			...
			//a point is in an area and that starting an AAS_TraceUserBBox from that
	...
	if (ground_foundreach)
	{
		if (ground_bestdist < 0)
		{
			...
			if (!aassettings.rs_maxfallheight || fabs(ground_bestdist) < aassettings.rs_maxfallheight) {
				...
				trace = AAS_TraceUserBBox(start, end, PRESENCE_NORMAL, -1);
...
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		testend[2] -= 100;
		trace = AAS_TraceUserBBox(teststart, testend, PRESENCE_NORMAL, -1);
		//
		if (trace.startsolid)
			return qfalse;
		if (trace.fraction < 1)
		...
		for (i = 0; i < 3; i++)
		{
			...
			//
			AAS_PredictUserMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue,
...
//#endif REACH_DEBUG
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num))
		{
			...
			trace = AAS_TraceUserBBox(destorigin, end, PRESENCE_CROUCH, -1);
			if (trace.startsolid)
			{
			...
			else*/
			{
				...
				VectorClear(cmdmove);
				AAS_PredictUserMovement(&move, -1, destorigin, PRESENCE_NORMAL, qfalse,
		...
		VectorScale(mid, 0.5, mid);
		//link an invalid (-1) entity
		areas = AAS_LinkEntityUserBBox(mins, maxs, -1, PRESENCE_CROUCH);
		...
		*/
		...
		for (i = 0; i < 2; i++)
		{
			...
			for (startreach = firststartreach; startreach; startreach = nextstartreach)
			{
				...
				//
				//trace = AAS_TraceUserBBox(startreach->start, move_start_top, PRESENCE_NORMAL, -1);
				...
				for (endreach = firstendreach; endreach; endreach = nextendreach)
				{
					...
					//
					//trace = AAS_TraceUserBBox(endreach->end, move_end_top, PRESENCE_NORMAL, -1);
					//if (trace.fraction < 1) continue;
					//
...
//		VectorScale(velocity, speed, velocity);
        ...
        */
		...
		if (velocity[0] || velocity[1])
		{
			...
			for (i = 0; i < 20; i++)
			{
				AAS_PredictUserMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse,
										velocity, cmdmove, 0, 30, 0.1f,
										SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME|
		...
		for (n = 0; n < 1/*2*/; n++)
		{
			...
			if (ret && speed < 300)
			{
				...
				//if (hordist < 1.6 * (facecenter[2] - areastart[2]))
				{
					...
					*/
					//
					AAS_PredictUserMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue,
												velocity, cmdmove, 30, 30, 0.1f,
												SE_ENTERWATER|SE_ENTERSLIME|
												SE_ENTERLAVA|SE_HITGROUNDDAMAGE|
			...
			for (j = 0; j < area->numfaces; j++)
			{
				...
				for (l = 0; l < face2->numedges; l++)
				{
					...
					if (abs(edge1num) == abs(edge2num))
					{
						...
						testend[2] -= 1000;
						trace = AAS_TraceUserBBox(mid, testend, PRESENCE_CROUCH, -1);
						//
						if (trace.startsolid)
						{
