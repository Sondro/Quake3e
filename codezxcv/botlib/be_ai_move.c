...
typedef struct bot_movestate_s
{
	...
	int user;									//user number of the bot
...
bot_movestate_t *botmovestates[MAX_USERS+1];
...
// Changes Globals:		-
//===========================================================================
int BotReachabilityArea(vec3_t origin, int user)
{
	...
	bsptrace = AAS_Trace(origin, mins, maxs, end, user, CONTENTS_SOLID|CONTENTS_PLAYERCLIP);
	if (!bsptrace.startsolid && bsptrace.fraction < 1 && bsptrace.ent != ENTITYNUM_NONE)
	{
		...
		trace = AAS_TraceUserBBox(org, end, PRESENCE_CROUCH, -1);
		if (!trace.startsolid)
		{
...
int BotReachabilityArea(vec3_t origin, int testground)
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (i > 0)
		{
			...
			trace = AAS_TraceUserBBox(origin, end, PRESENCE_CROUCH, -1);
			if (!trace.startsolid)
			{
				VectorCopy(trace.endpos, org);
			} //end if
...
float BotGapDistance(vec3_t origin, vec3_t hordir, int entnum)
{
	...
	//this enables walking down stairs more fluidly
	{
		...
		trace = AAS_TraceUserBBox(start, end, PRESENCE_CROUCH, entnum);
...
int BotCheckBarrierJump(bot_movestate_t *ms, vec3_t dir, float speed)
{
	...
	//trace right up
	trace = AAS_TraceUserBBox(ms->origin, end, PRESENCE_NORMAL, ms->entitynum);
	...
	trace = AAS_TraceUserBBox(start, end, PRESENCE_NORMAL, ms->entitynum);
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type)
{
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		...
		origin[2] += 0.5;
		AAS_PredictUserMovement(&move, ms->entitynum, origin, presencetype, qtrue,
									velocity, cmdmove, cmdframes, maxframes, 0.1f,
									stopevent, 0, qfalse);//qtrue);
		//if prediction time wasn't enough to fully predict the movement
		if (move.frames >= maxframes && (type & MOVE_JUMP))
		{
			//botimport.Print(PRT_MESSAGE, "user %d: max prediction frames\n", ms->user);
			return qfalse;
		} //end if
		//don't enter slime or lava and don't fall from too high
		if (move.stopevent & (SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE))
		{
			//botimport.Print(PRT_MESSAGE, "user %d: would be hurt ", ms->user);
		...
		if (type & MOVE_JUMP) EA_Jump(ms->user);
		if (type & MOVE_CROUCH) EA_Crouch(ms->user);
		EA_Move(ms->user, hordir, speed);
...
//	if (dist1 && dist1 <= trace.fraction * 80) VectorMA(reach->start, dist1-20, hordir, trace.endpos);
	...
	if (DotProduct(dir1, dir2) < -0.8 || dist2 < 5)
	{
		...
		VectorNormalize(hordir);
		//elemantary action jump
		if (dist1 < 24) EA_Jump(ms->user);
		else if (dist1 < 32) EA_DelayedJump(ms->user);
		EA_Move(ms->user, hordir, 600);
		//
		ms->jumpreach = ms->lastreachnum;
	} //end if
	else
...
bot_moveresult_t BotTravel_Teleport(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	if (dist < 30) EA_Move(ms->user, hordir, 200);
	else EA_Move(ms->user, hordir, 400);
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	else
	{
		...
		if (dist < 64)
		{
			...
			{
				if (speed > 5) EA_Move(ms->user, dir, speed);
			} //end if
			VectorCopy(dir, result.movedir);
			//
			if (ms->moveflags & MFL_SWIMMING) result.flags |= MOVERESULT_SWIMVIEW;
...
bot_moveresult_t BotFinishTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	if (fabs(bottomdir[2]) < fabs(topdir[2]))
	{
		...
		EA_Move(ms->user, bottomdir, 300);
	} //end if
	else
	{
		VectorNormalize(topdir);
		EA_Move(ms->user, topdir, 300);
	} //end else
	return result;
	...
	if ((reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_GRAPPLEHOOK)
	{
		if ((ms->moveflags & MFL_ACTIVEGRAPPLE) || ms->grapplevisible_time)
		{
			if (offhandgrapple->value)
				EA_Command(ms->user, cmd_grappleoff->string);
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	else
	{
			...
			fabs(AngleDiff(result.ideal_viewangles[1], ms->viewangles[1])) < 2)
		{
			...
			if (offhandgrapple->value)
			{
				EA_Command(ms->user, cmd_grappleon->string);
			} //end if
			else
			{
				EA_Attack(ms->user);
...
bot_moveresult_t BotTravel_RocketJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	EA_View(ms->user, result.ideal_viewangles);
	//view is important for the movement
	result.flags |= MOVERESULT_MOVEMENTVIEWSET;
	//select the rocket launcher
	EA_SelectWeapon(ms->user, (int) weapindex_rocketlauncher->value);
...
bot_moveresult_t BotTravel_BFGJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	result.flags |= MOVERESULT_MOVEMENTVIEWSET;
	//select the rocket launcher
	EA_SelectWeapon(ms->user, (int) weapindex_bfg10k->value);
	//weapon is used for movement
...
void BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags)
{
	...
	{
#ifdef DEBUG
		botimport.Print(PRT_MESSAGE, "user %d: movetogoal -> no goal\n", ms->user);
#endif //DEBUG
		result->failure = qtrue;
		return;
	} //end if
			...
			if (modelnum >= 0 && modelnum < MAX_MODELS)
			{
				...
				if (modeltype == MODELTYPE_FUNC_PLAT)
				{
						...
						(reach.facenum & 0x0000FFFF) != modelnum)
					{
						...
						if (reachnum)
						{
							//botimport.Print(PRT_MESSAGE, "user %d: accidentally ended up on func_plat\n", ms->user);
							AAS_ReachabilityFromNum(reachnum, &reach);
						...
						else
						{
							if (botDeveloper)
							{
								botimport.Print(PRT_MESSAGE, "user %d: on func_plat without reachability\n", ms->user);
							} //end if
							result->blocked = qtrue;
				...
				else if (modeltype == MODELTYPE_FUNC_BOB)
				{
						...
						(reach.facenum & 0x0000FFFF) != modelnum)
					{
						...
						if (reachnum)
						{
							//botimport.Print(PRT_MESSAGE, "user %d: accidentally ended up on func_bobbing\n", ms->user);
							AAS_ReachabilityFromNum(reachnum, &reach);
							ms->lastreachnum = reachnum;
							ms->reachability_time = AAS_Time() + BotReachabilityTime(&reach);
						} //end if
						else
						{
							if (botDeveloper)
							{
								botimport.Print(PRT_MESSAGE, "user %d: on func_bobbing without reachability\n", ms->user);
							} //end if
							result->blocked = qtrue;
							result->blockentity = ent;
							result->flags |= MOVERESULT_ONTOPOFOBSTACLE;
	...
	if (ms->moveflags & (MFL_ONGROUND|MFL_SWIMMING|MFL_AGAINSTLADDER))
	{
		...
		if (reachnum)
		{
			...
			else
			{
				...
				if (botDeveloper)
				{
					if (ms->reachability_time < AAS_Time())
					{
						botimport.Print(PRT_MESSAGE, "user %d: reachability timeout in ", ms->user);
		...
		if (reachnum)
		{
			...
			//
#ifdef DEBUG
			//botimport.Print(PRT_MESSAGE, "user %d: ", ms->user);
		...
		if (botDeveloper)
		{
			if (result->failure)
			{
				botimport.Print(PRT_MESSAGE, "user %d: movement failure in ", ms->user);
				AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
				botimport.Print(PRT_MESSAGE, "\n");
			} //end if
		} //end if
	...
	else
	{
		...
		for (i = numareas-1; i >= 0; i--)
		{
			...
			{
				//botimport.Print(PRT_MESSAGE, "user %d used a jumppad without knowing, area %d\n", ms->user, areas[i]);
				foundjumppad = qtrue;
		...
		if (botDeveloper)
		{
			...
			{
				botimport.Print(PRT_MESSAGE, "user %d didn't find jumppad reachability\n", ms->user);
			} //end if
		} //end if
		//
		if (ms->lastreachnum)
		{
			...
			result->traveltype = reach.traveltype;
#ifdef DEBUG
			//botimport.Print(PRT_MESSAGE, "user %d finish: ", ms->user);
			//AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
			//botimport.Print(PRT_MESSAGE, "\n");
#endif //DEBUG
			//
			...
			if (botDeveloper)
			{
				if (result->failure)
				{
					botimport.Print(PRT_MESSAGE, "user %d: movement failure in finish ", ms->user);
					AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
					botimport.Print(PRT_MESSAGE, "\n");
				} //end if
			} //end if
