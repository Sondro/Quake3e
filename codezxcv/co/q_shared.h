...
#endif
#define USER_WINDOW_TITLE   "Quake 3: Arena"
...
#define MAX_MASTER_HOSTS      5	// number of supported master hosts
...
#define MAX_NAME_LENGTH		32		// max length of a user name
...
// print levels from gfxer (FIXME: set up for game / app?)
typedef enum {
	PRINT_ALL,
...
typedef enum {
	...
	ERR_HOSTDISCONNECT,		// don't kill host
	ERR_DISCONNECT,				// user disconnected from the host
	ERR_NEED_CD					// pop up the need-cd dialog
} errorParm_t;


// font drawing values used by ui and app
...
#define CVAR_USERINFO		0x0002	// sent to host on connect or change
#define CVAR_HOSTINFO		0x0004	// sent in response to front end requests
#define CVAR_SYSTEMINFO		0x0008	// these cvars will be duplicated on all users
...
#define CVAR_HOST_CREATED	0x0800	// cvar was created by a host the user connected to.
#define CVAR_VM_CREATED		0x1000	// cvar was created exclusively in one of the VMs.
#define CVAR_PROTECTED		0x2000	// prevent modifying this var from VMs or the host
...
#define SNAPFLAG_HOSTCOUNT	4	// toggled every map_restart so transitions can be detected

//
// per-level limits
//
#define MAX_USERS			64		// absolute limit
...
// other ones are strictly for hostgame to usergame communication
#define CS_HOSTINFO		0		// an info string with all the hostinfo cvars
#define CS_SYSTEMINFO		1		// an info string for host system to user system configuration (timescale, etc)
...
// playerState_t is the information needed by both the user and host
...
typedef struct playerState_s {
	int			commandTime;	// cmd->hostTime of last executed command
	int			pm_type;
	...
	int			userNum;		// ranges from 0 to MAX_USERS-1
	...
	int			ping;			// host to game info for scoreboard
...
//
// usercmd_t->button bits, many of which are generated by the user system,
// so they aren't game/app only definitions
//
...
// usercmd_t is sent to the host each user frame
...
// entityState_t is the information conveyed from the host
// in an update message about entities that the user will
// need to draw in some way
...
typedef struct entityState_s {
	...
	int		modelindex;
	int		modelindex2;
	int		userNum;		// 0 to (MAX_USERS - 1), for players and corpses
	int		frame;

	int		solid;			// for user side prediction, trap_linkentity sets this properly
...
typedef enum {
	CA_UNINITIALIZED,
	CA_DISCONNECTED, 	// not talking to a host
	CA_AUTHORIZING,		// not used any more, was checking cd key 
	CA_CONNECTING,		// sending request packets to the host
	CA_CHALLENGING,		// sending challenge packets to the host
	CA_CONNECTED,		// netchan_t established, getting gamestate
	CA_LOADING,			// only during app initialization, never during main loop
	CA_PRIMED,			// got gamestate, waiting for first frame
	CA_ACTIVE,			// game views should be displayed
	CA_CINEMATIC		// playing a cinematic or a static pic, not connected to a host
} connstate_t;
...
#define MAX_GLOBAL_HOSTS				4096
#define MAX_OTHER_HOSTS					128
#define MAX_PINGREQUESTS					32
#define MAX_HOSTSTATUSREQUESTS	16
