...
void MSG_WriteString( msg_t *sb, const char *s) {
	...
	for ( i = 0 ; i < l; i++) {
		// get rid of 0x80+ and '%' chars, because old users don't like them
		if ( s[i] & 0x80 || s[i] == '%')
			v = '.';
		else
			v = s[i];
...
void MSG_WriteDeltaUsercmdKey( msg_t *msg, int key, const usercmd_t *from, const usercmd_t *to) {
	if ( (unsigned)(to->hostTime - from->hostTime) < 256) {
		MSG_WriteBits( msg, 1, 1);
		MSG_WriteBits( msg, to->hostTime - from->hostTime, 8);
	} else {
		MSG_WriteBits( msg, 0, 1);
		MSG_WriteBits( msg, to->hostTime, 32);
	}
...
void MSG_ReadDeltaUsercmdKey( msg_t *msg, int key, const usercmd_t *from, usercmd_t *to) {
	if ( MSG_ReadBits( msg, 1)) {
		to->hostTime = from->hostTime + MSG_ReadBits( msg, 8);
	} else {
		to->hostTime = MSG_ReadBits( msg, 32);
	}
	if ( MSG_ReadBits( msg, 1)) {
		key ^= to->hostTime;
