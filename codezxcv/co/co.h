...
===========================================================================
*/
// co.h -- definitions common between user and host, but not game.or ref modules
#ifndef _CO_H_
#define _CO_H_
...
#define PACKET_BACKUP	32	// number of old messages that must be kept on user and
							// host for delta comrpession and ping estimation
...
#define MAX_DOWNLOAD_WINDOW		48	// ACK window of 48 download chunks. Cannot set this higher, or users
...
#define UPDATE_HOST_NAME	"update.quake3arena.com"
// override on command line, config files etc.
#ifndef MASTER_HOST_NAME
#define MASTER_HOST_NAME	"master.quake3arena.com"
#endif
#ifndef AUTHORIZE_HOST_NAME
#define AUTHORIZE_HOST_NAME	"authorize.quake3arena.com"
#endif
...
#endif
#define PORT_HOST			27960
#define NUM_HOST_PORTS	4		// broadcast scan this many ports after
									// PORT_HOST so a single machine can
									// run multiple hosts
...
enum svc_ops_e {
	...
	svc_hostCommand,			// [string] to be executed by user game module
...
enum clc_ops_e {
	...
	clc_userCommand,		// [string] message
	clc_EOF,
...
// module should be bare: "app", not "app.dll" or "vm/app.qvm"
...
// if function is NULL, the command will be forwarded to the host
// as a clc_userCommand instead of executed locally
...
// in their flags (CVAR_USERINFO, CVAR_HOSTINFO, CVAR_SYSTEMINFO, etc)
...
// a single check can determine if any CVAR_USERINFO, CVAR_HOSTINFO,
...
#ifdef DEDICATED
#define Q3CONFIG_CFG "q3config_host.cfg"
#define CONSOLE_HISTORY_FILE "q3history_host"
#else
...
qboolean	FS_ConditionalRestart(int checksumFeed, qboolean userRestart);
...
// Hosts with sv_pure set will get this string and pass it to users.
...
// AND referenced pk3 files. Hosts with sv_pure set will get this string 
// back from users for pure validation 

void FS_ClearPakReferences(int flags);
// clears referenced booleans on loaded pk3s

void FS_PureHostSetReferencedPaks(const char *pakSums, const char *pakNames);
void FS_PureHostSetLoadedPaks(const char *pakSums, const char *pakNames);
...
// customizable user window title
...
// both user and host must agree to pause
...
extern	int		time_backend;		// gfxer backend time
...
host clipmap
---mark---
gfxer initialization (shaders, etc)
UI vm
app vm
gfxer map
gfxer models
...
// when the host is going to load a new map, the entire hunk
// will be cleared, so the user must shutdown app, ui, and
// the gfxer

void	CL_ForwardCommandToHost(const char *string);
// adds the current command line as a clc_userCommand to the user message.
...
// start all the user stuff using the hunk
...
// the system console is shown when a dedicated host is running
