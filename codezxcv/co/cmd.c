...
void Cmd_ExecuteString( const char *text ) {
...
#ifndef DEDICATED
	// check user game commands
	if ( com_cl_running && com_cl_running->integer && CL_GameCommand() ) {
		return;
	}
#endif

	// check host game commands
	...
	// send it as a host command if we are connected
	// this will usually result in a chat message
	CL_ForwardCommandToHost( text );
#endif
}
