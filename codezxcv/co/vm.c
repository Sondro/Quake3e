...
void VM_Init( void) {
	...
	Cvar_Get( "vm_app", "2", CVAR_ARCHIVE | CVAR_PROTECTED);	// !@# SHIP WITH SET TO 2
#endif
...
void VM_ReplaceInstructions( vm_t *vm, instruction_t *buf) {
	...
	if ( vm->index == VM_GAME) {
		if ( vm->crc32sum == 0x5AAE0ACC && vm->instructionCount == 251521 && vm->exactDataLength == 1872720) {
			vm->forceDataMask = qtrue; // OSP host doing some bad things with memory
		} else {
...
This allows a host to do a map_restart without changing memory allocation
=================
*/
...
static void VM_VmProfile_f( void) {
	...
	if ( Cmd_Argc() < 2) {
		Com_Printf( "usage: %s <game|app|ui>\n", Cmd_Argv( 0));
		return;
	}
