...
sometimes remap the user's source port on a packet during gameplay.
...
void Netchan_TransmitNextFragment( netchan_t *chan) {
	...
	// send the qport if we are a user
	if ( chan->sock == NS_USER) {
...
//	Netchan_UnScramblePacket( msg);
	...
	// read the qport if we are a host
	if ( chan->sock == NS_HOST) {
	...
	if ( fragmented) {
		...
		*(int32_t *)msg->data = LittleLong( sequence);
		...
		// TTimo
		// users were not acking fragmented messages
...
#ifndef DEDICATED
	if ( sock == NS_USER && cl_packetdelay->integer > 0) {
		NET_QueuePacket( length, data, to, cl_packetdelay->integer);
	} else
#endif
	if ( sock == NS_HOST && sv_packetdelay->integer > 0) {
