...
==================
*/
void CM_LoadMap(const char *name, qboolean userload, int *checksum) {
	void			*buf;
	int				i;
	dheader_t		header;
	int				length;
	...
	Com_DPrintf("%s('%s', %i)\n", __func__, name, userload);

	if(!strcmp(cm.name, name) && userload) {
	...
	*checksum = cm.checksum = LittleLong(Com_BlockChecksum(buf, length));
	...
	// allow this to be cached if it is loaded by the host
	if(!userload) {
