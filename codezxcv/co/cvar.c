...
cvar_t *Cvar_Get( const char *var_name, const char *var_value, int flags) {
	...
	if(var)
	{
		...
		// Make sure hosts cannot mark engine-added variables as HOST_CREATED
		if ( var->flags & CVAR_HOST_CREATED)
		{
			if ( !( flags & CVAR_HOST_CREATED)) {
				// reset host-created flag
				var->flags &= ~CVAR_HOST_CREATED;
				if ( vm_created) {
	...
	var->flags = flags;
	// note what types of cvars have been modified (userinfo, archive, hostinfo, systeminfo)
...
void Cvar_SetSafe( const char *var_name, const char *value)
{
	...
	if ( flags != CVAR_NONEXISTENT)
	{
		...
		// don't let VMs or host change engine latched cvars instantly
...
static void Cvar_Set_f( void) {
	...
	switch( cmd[3]) {
		...
		case 'u':
			...
			break;
		case 's':
			if( !( v->flags & CVAR_HOSTINFO)) {
				v->flags |= CVAR_HOSTINFO;
...
This will only accept to run when both the host and user are running unless forced
============
*/
...
#ifdef DEDICATED	
	Com_Printf( S_COLOR_YELLOW " You're not running a host, so not all subsystems/VMs are loaded.\n");
#else
	Com_Printf( S_COLOR_YELLOW " You're not running a listen host, so not all subsystems/VMs are loaded.\n");
#endif
...
=====================
*/
#define INVALID_FLAGS ( CVAR_USER_CREATED | CVAR_HOST_CREATED | CVAR_PROTECTED | CVAR_PRIVATE | CVAR_MODIFIED | CVAR_NONEXISTENT)
