...
===========================================================================
*/
// common.c -- misc functions used in user and host
...
// com_speeds times
int		time_game;
int		time_frontend;		// gfxer frontend time
int		time_backend;		// gfxer backend time
...
// gfxer window states
qboolean	gw_minimized = qfalse; // this will be always true for dedicated hosts
#ifndef DEDICATED
...
Both user and host can use this, and it will output
...
#ifndef DEDICATED
	// echo to user console if we're not a dedicated host
...
Both user and host can use this, and it will
...
void QDECL Com_Error(errorParm_t code, const char *fmt, ...) {
	...
	if(code != ERR_DISCONNECT && code != ERR_NEED_CD) {
		...
		// will trigger recursive error without proper user/host shutdown
		if(code != ERR_FATAL) {
	...
	if(code == ERR_DISCONNECT || code == ERR_HOSTDISCONNECT) {
		VM_Forced_Unload_Start();
		SV_Shutdown("Host disconnected");
		Com_EndRedirect();
#ifndef DEDICATED
		...
		Com_Printf("********************\nERROR: %s\n********************\n",
			com_errorMessage);
		VM_Forced_Unload_Start();
		SV_Shutdown(va("Host crashed: %s",  com_errorMessage));
		Com_EndRedirect();
#ifndef DEDICATED
		CL_Disconnect(qfalse);
		CL_FlushMemory();
		...
		SV_Shutdown("Host didn't have CD");
		Com_EndRedirect();
#ifndef DEDICATED
		if(com_cl_running && com_cl_running->integer) {
			...
			Com_Printf("Host didn't have CD\n");
		}
#endif
		FS_PureHostSetLoadedPaks("", "");
		com_errorEntered = qfalse;
		...
		VM_Forced_Unload_Start();
#ifndef DEDICATED
		CL_Shutdown(va("Host fatal crashed: %s", com_errorMessage), qtrue);
#endif
		SV_Shutdown(va("Host fatal crashed: %s", com_errorMessage));
		Com_EndRedirect();
		VM_Forced_Unload_Done();
	}
...
void Com_Quit_f(void) {
	...
	if(!com_errorEntered) {
		...
		VM_Forced_Unload_Start();
		SV_Shutdown(p[0] ? p : "Host quit");
#ifndef DEDICATED
		CL_Shutdown(p[0] ? p : "User quit", qtrue);
#endif
		VM_Forced_Unload_Done();
		Com_Shutdown();
		FS_Shutdown(qtrue);
...
char cl_title[ MAX_CVAR_VALUE_STRING ] = USER_WINDOW_TITLE;
...
Goals:
	...
	allow restarting of the user without fragmentation
...
static void Zone_Stats(const char *name, const memzone_t *z, qboolean printDetails, zone_stats_t *stats)
{
	...
	for(block = zone->blocklist.next ; ;) {
		...
		if(block->tag != TAG_FREE) {
			...
			} else if(block->tag == TAG_GFXER) {
				st.gfxerBytes += block->size;
			}
		} else {
...
static void Com_Meminfo_f(void) {
	...
	Com_Printf("        %8i bytes in gfxer\n", st.gfxerBytes);
	Com_Printf("        %8i bytes in other\n", st.zoneBytes -(st.botlibBytes + st.gfxerBytes));
...
The host calls this after the level and game VM have been loaded
===================
*/
...
The user calls this before starting a vid_restart or snd_restart
=================
*/
...
The host calls this before shutting down or loading a new map
=================
*/
...
/*
=================
Com_RunAndTimeHostPacket
=================
*/
void Com_RunAndTimeHostPacket(const netadr_t *evFrom, msg_t *buf) {
	int		t1, t2, msec;
...
#ifndef DEDICATED
			while(NET_GetLoopPacket(NS_USER, &evFrom, &buf)) {
				CL_PacketEvent(&evFrom, &buf);
			}
#endif
			while(NET_GetLoopPacket(NS_HOST, &evFrom, &buf)) {
				// if the host just shut down, flush the events
				if(com_sv_running->integer) {
					Com_RunAndTimeHostPacket(&evFrom, &buf);
				}
			}
...
==================
*/
void Com_GameRestart(int checksumFeed, qboolean userRestart)
{
...
// TTimo: centralizing the cl_cdkey stuff after I discovered a buffer overflow problem with the dedicated host version
...
//	Swap_Init();
	...
	Cvar_Get("sv_master1", MASTER_HOST_NAME, CVAR_INIT);
	Cvar_Get("sv_master2", "master.ioquake3.org", CVAR_INIT);
	Cvar_Get("sv_master3", "master.maverickhosts.com", CVAR_INIT);
	...
	com_protocol->flags |= CVAR_HOSTINFO | CVAR_ROM;
	...
	com_version = Cvar_Get("version", s, CVAR_PROTECTED | CVAR_ROM | CVAR_HOSTINFO);
	...
	// being random enough for a hostid
...
static int Com_ModifyMsec(int msec) {
	...
	if(com_dedicated->integer) {
		// dedicated hosts don't want to clamp for a much longer
		// period, because it would mess up all the user's views
		// of time.
	...
	if(!com_sv_running->integer) {
		// users of remote hosts do not want to clamp time, because
		// it would skew their view of the host's time temporarily
		clampTime = 5000;
	} else {
...
void Com_Frame(qboolean noDelay) {
	...
	if(com_dedicated->integer) {
		...
		if(noDelay) {
			...
			// Adjust minMsec if previous frame took too long to gfx so
	...
	//
	if(com_speeds->integer) {
		timeBeforeHost = Sys_Milliseconds();
	}

	SV_Frame(msec);

	// if "dedicated" has been modified, start up
	// or shut down the user system.
	// Do this after the host may have started,
	// but before the user tries to auto-connect
	...
	if(!com_dedicated->integer) {
		//
		// run event loop a second time to get host to user packets
		...
		//
		if(com_speeds->integer) {
			timeBeforeUser = Sys_Milliseconds();
		}
	...
	if(com_speeds->integer) {
		...
		all = timeAfter - timeBeforeHost;
		sv = timeBeforeEvents - timeBeforeHost;
		ev = timeBeforeHost - timeBeforeFirstEvents + timeBeforeUser - timeBeforeEvents;
		cl = timeAfter - timeBeforeUser;
		sv -= time_game;
