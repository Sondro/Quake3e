...
because in-game console can't properly gfxer non-ascii characters
...
LAN users will have their rate var ignored
==================
*/
...
static void NET_OpenSocks(int port) {
	...
	Com_Printf("Opening connection to SOCKS host.\n");
	...
	h = gethostbyname(net_socksHost->string);
	if(h == NULL) {
...
static void NET_OpenIP(void) {
	...
	// dedicated hosts can be started without requiring
...
static qboolean NET_GetCvars(void) {
	...
	// I want host owners to explicitly turn on ipv6 support.
	...
	net_port = Cvar_Get("net_port", va("%i", PORT_HOST), CVAR_LATCH | CVAR_NORESTART);
	...
	net_port6 = Cvar_Get("net_port6", va("%i", PORT_HOST), CVAR_LATCH | CVAR_NORESTART);
	Cvar_CheckRange(net_port6, "0", "65535", CV_INTEGER);
	modified += net_port6->modified;
	net_port6->modified = qfalse;

	// Some cvars for configuring multicast options which facilitates scanning for hosts on local subnets.
	...
	net_socksHost = Cvar_Get("net_socksHost", "", CVAR_LATCH | CVAR_ARCHIVE_ND);
	modified += net_socksHost->modified;
	net_socksHost->modified = qfalse;
...
#ifdef DEDICATED
			Com_RunAndTimeHostPacket(&from, &netmsg);
#else
