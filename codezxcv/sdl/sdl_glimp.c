...
static int GLW_SetMode(int mode, const char *modeFS, qboolean fullscreen, qboolean vulkan)
{
	...
	if(SDL_window)
	{
		...
		SDL_Surface *icon = SDL_CreateRGBSurfaceFrom(
			(void *)USER_WINDOW_ICON.pixel_data,
			USER_WINDOW_ICON.width,
			USER_WINDOW_ICON.height,
			USER_WINDOW_ICON.bytes_per_pixel * 8,
			USER_WINDOW_ICON.bytes_per_pixel * USER_WINDOW_ICON.width,
	...
	// save draw dimensions as gfxer may change it in advance
...
void GLimp_Init(glconfig_t *config)
{
	...
	glw_state.config = config; // feedback gfxer configuration
...
Used by opengl gfxers to resolve all qgl* function pointers
===============
*/
...
void VKimp_Init(glconfig_t *config)
{
	...
	// feedback to gfxer configuration
